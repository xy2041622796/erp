...
export type SalaryRuleType = 'ATTENDANCE' | 'HOUSING_FUND' | 'SOCIAL' | 'TAX';
...
export type SalaryRuleBundle = {
  attendanceSettlementRules: AttendanceSettlementRule[];
...
function normalizeAttendanceSettlementRule(row: SalaryRuleRow): AttendanceSettlementRule {
  const parsed = parseRuleJson<Partial<AttendanceSettlementRule>>(row, {});
  return {
    code: normalizeText(parsed.code || row.rule_code),
    title: normalizeText(parsed.title || row.rule_name),
    description: normalizeText(parsed.description || row.description),
    overtimeMultiplierWorkday: Number(parsed.overtimeMultiplierWorkday ?? 1.5),
    sickLeaveDeductRate: Number(parsed.sickLeaveDeductRate ?? 0.5),
    personalLeaveDeductRate: Number(parsed.personalLeaveDeductRate ?? 1),
    annualLeaveDeductRate: Number(parsed.annualLeaveDeductRate ?? 0),
    absentDeductRate: Number(parsed.absentDeductRate ?? 1),
    lateEarlyFixedAmount: Number(parsed.lateEarlyFixedAmount ?? 20),
    dailySalaryDays: Number(parsed.dailySalaryDays ?? 21.75),
    workHoursPerDay: Number(parsed.workHoursPerDay ?? 8),
    overtimeItemCode: normalizeText(parsed.overtimeItemCode),
    attendanceDeductionItemCode: normalizeText(parsed.attendanceDeductionItemCode),
  };
}
...
function buildRuleRow(ruleType: SalaryRuleType, rule: any, existed?: SalaryRuleRow): SalaryRuleRow {
  ...
  const baseItemCodes = ruleType === 'ATTENDANCE' ? [] : normalizeBaseItemCodes(rule.baseItemCodes);
  return {
    ...
    base_item_codes: baseItemCodes.join(','),
...
export async function getSalaryRuleBundle(): Promise<SalaryRuleBundle> {
  const rows = await querySalaryRuleRows(['ATTENDANCE', 'TAX', 'SOCIAL', 'HOUSING_FUND']);
  const attendanceRows = rows.filter((item) => item.rule_type === 'ATTENDANCE');
  ...
  return {
    attendanceSettlementRules: attendanceRows.length ? attendanceRows.map(normalizeAttendanceSettlementRule) : deepClone(defaultAttendanceSettlementRules),
...
export async function saveSalaryRuleBundle(bundle: SalaryRuleBundle) {
  await saveRulesByType('ATTENDANCE', bundle.attendanceSettlementRules || []);
...
export async function resetSalaryRulesByType(ruleType: SalaryRuleType) {
  if (ruleType === 'ATTENDANCE') {
    await saveRulesByType('ATTENDANCE', deepClone(defaultAttendanceSettlementRules));
  }
  if (ruleType === 'TAX') {
