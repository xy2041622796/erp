...
function createOpeningProductStockTable(
  formId = OPENING_PRODUCT_STOCK_FORM_ID,
) {
...
export async function createOpeningProductStock(
  data: OpeningProductStockApi.Row,
) {
  const table = createOpeningProductStockTable();
  const payload = {
    ...data,
...
export async function updateOpeningProductStock(
  data: OpeningProductStockApi.Row,
) {
  const table = createOpeningProductStockTable();
  const payload = {
...
export async function deleteOpeningProductStock(id: string) {
  return await updateOpeningProductStock({
    rowid: id,
