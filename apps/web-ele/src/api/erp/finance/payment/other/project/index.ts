...
export namespace ErpOtherExpenseProjectApi {
...
export async function getOtherExpenseProjectPage(params: any) {
...
export async function getOtherExpenseProjectSimpleList() {
  const table = createFinanceDataTable(PROJECT_MODEL_ID, PROJECT_TABLE, PROJECT_DB, PROJECT_PK);
...
export async function getOtherExpenseProject(id: string) {
  const table = createFinanceDataTable(PROJECT_MODEL_ID, PROJECT_TABLE, PROJECT_DB, PROJECT_PK);
  table.Filter = and(cond('lingma_sys_is_delete', 'equal', 0), cond(PROJECT_PK, 'equal', id));
