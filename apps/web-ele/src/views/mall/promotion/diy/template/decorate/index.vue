...
import type { DiyComponentLibrary } from '#/views/mall/promotion/components/diy-editor/util'; // 商城的 DIY 组件，在 DiyEditor 目录下
...
import DiyEditor from '#/views/mall/promotion/components/diy-editor/index.vue';
import { PAGE_LIBS } from '#/views/mall/promotion/components/diy-editor/util';
...
import {
  ...
  ElRadioGroup,
  ElTooltip,
} from 'element-plus'; // 特殊：存储 reset 重置时，当前 selectedTemplateItem 值，从而进行恢复
...
function recoverPageIndex() {
  // 恢复重置前的页面，默认是第一个页面
  const index = Number(sessionStorage.getItem(DIY_PAGE_INDEX_KEY)) || 0;
  // 移除标记
  ...
  >();
  // 切换页面
  if (index !== selectedTemplateItem.value) {
    handleTemplateItemChange(index);
  }
}
