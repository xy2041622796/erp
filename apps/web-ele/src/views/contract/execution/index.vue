...
import { ElAlert, ElRadioButton, ElRadioGroup, ElTag } from 'element-plus';

const executionType = ref<'payment' | 'invoice' | 'acceptance'>('payment');
...
<template>
  <Page auto-content-height>
    ...
    <div class="mb-4 flex flex-wrap items-center gap-4">
      ...
      <ElRadioGroup v-model="executionType">
        <ElRadioButton label="payment">收付款履行</ElRadioButton>
        <ElRadioButton label="invoice">开票履行</ElRadioButton>
        <ElRadioButton label="acceptance">验收履行</ElRadioButton>
      </ElRadioGroup>
    </div>
    <div class="mb-4">
    ...
    <Grid>
      ...
      <template #settle_amount="{ row }">
        <div class="flex flex-col text-right">
          <span>{{ formatAmount(pickNumber(row, ['settled_amount', 'settle_amount'])) }}</span>
          <span class="text-xs text-gray-500">
            {{ formatAmount(Number((row as any).contract_total_amount ?? 0) - pickNumber(row, ['settled_amount', 'settle_amount'])) }}
          </span>
        </div>
      </template>
      ...
      <template #receive_amount="{ row }">
        <div class="flex flex-col text-right">
          <span>{{ formatAmount(pickNumber(row, ['received_amount', 'receive_amount', 'paid_amount'])) }}</span>
          <span class="text-xs text-gray-500">
            {{ formatAmount(Number((row as any).contract_total_amount ?? 0) - pickNumber(row, ['received_amount', 'receive_amount', 'paid_amount'])) }}
          </span>
        </div>
      </template>
      ...
      <template #invoice_amount="{ row }">
        <div class="flex flex-col text-right">
          <span>{{ formatAmount(pickNumber(row, ['invoice_amount', 'invoiced_amount'])) }}</span>
          <span class="text-xs text-gray-500">
            {{ formatAmount(Number((row as any).contract_total_amount ?? 0) - pickNumber(row, ['invoice_amount', 'invoiced_amount'])) }}
          </span>
        </div>
      </template>
