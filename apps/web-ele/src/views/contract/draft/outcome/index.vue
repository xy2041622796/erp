...
import { downloadContractImportTemplate, deleteContract, exportContract, getContract, getContractPage, importContract } from '#/api/erp/contract/outcome';
...
import SettlementForm from '#/views/finance/payment/settlement/modules/form.vue';
...
async function handleImportSubmit() {
  ...
  try {
    await importContract(selectedImportFile.value, OUTCOME_CONTRACT_EXPORT_ENCODING_ID);
...
function shouldShowChangePlan() {
  return changeForm.value.change_type === '付款计划变更' || changeForm.value.change_type === '付款明细变更' || changeForm.value.change_type === '综合变更';
}
...
function validateChangePlan() {
  ...
  if (Math.abs(total - targetAmount) > 0.01) {
    ElMessage.warning('付款计划合计必须等于变更后金额，当前合计：' + total.toFixed(2));
    return false;
  }
  return true;
}
...
async function handleSettlement(row: CrmContractApi.Contract) {
  ...
  try {
    const res = await getExpenseSettlementPage({
      pageNo: 1,
      page: 50,
      contract_id: contractId,
      settlement_type: 0,
  ...
  <Page auto-content-height>
    ...
    <ElDialog v-model="changeDialogVisible" title="合同变更" width="760px" append-to-body>
      <ElForm label-width="110px">
        ...
        <div class="grid grid-cols-2 gap-4">
          <ElFormItem label="变更类型" required>
            <ElSelect v-model="changeForm.change_type" class="w-full">
              ...
              <ElOption label="付款计划变更" value="付款计划变更" />
        ...
        <ElFormItem v-if="shouldShowChangePlan()" label="付款计划">
          <div class="w-full">
