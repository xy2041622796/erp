...
function handleSuccess(savedItem: CrmProductCategoryApi.ProductCategory) {
  const fullData = [...gridApi.grid.getTableData().fullData];
  const index = fullData.findIndex((item: any) => item.id === savedItem.id);
  if (index !== -1) {
    fullData[index] = { ...fullData[index], ...savedItem };
  } else {
    fullData.push(savedItem);
  }
  gridApi.grid.loadData(fullData);
}
