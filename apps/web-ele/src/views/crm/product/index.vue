...
async function handleDelete(row: CrmProductApi.Product) {
  ...
  try {
    await deleteProduct(row.rowid || row.id!);
    ...
    <Grid table-title="产品列表">
      ...
      <template #name="{ row }">
        <ElButton
          type="primary"
          link
          :disabled="!dataTable?.hasShowField(row.rowid)"
          ...
          :actions="[
            {
              ...
              disabled: !dataTable?.authCheck.getRowAuth(row.rowid).allowEdit,
            ...
            },
            {
              ...
              disabled: !dataTable?.authCheck.getRowAuth(row.rowid).allowDelete,
              popConfirm: {
