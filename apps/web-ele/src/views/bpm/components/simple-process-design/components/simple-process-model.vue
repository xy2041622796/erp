...
<template>
    ...
    <div class="absolute right-0 top-0 bg-card">
      <ElRow type="flex" justify="end">
        <ElButtonGroup key="scale-control">
          ...
          <ElButton v-if="!readonly" @click="importJson">
          ...
          />
          <ElButton @click="processReZoom()">
            <IconifyIcon icon="lucide:table-columns-split" />
          </ElButton>
          <ElButton :plain="true" @click="zoomOut()">
            <IconifyIcon icon="lucide:zoom-out" />
          </ElButton>
          <ElButton class="w-20"> {{ scaleValue }}% </ElButton>
          <ElButton :plain="true" @click="zoomIn()">
            <IconifyIcon icon="lucide:zoom-in" />
          </ElButton>
          <ElButton @click="resetPosition">重置</ElButton>
        </ElButtonGroup>
      </ElRow>
    </div>
    <div
  ...
  <ElDialog v-model="errorDialogVisible" title="保存失败" width="400">
    ...
    <template #footer>
      <ElButton type="primary" @click="errorDialogVisible = false">
        知道了
      </ElButton>
    </template>
  </ElDialog>
