...
const DISPLAY_GROUP_COLUMN_INDEX = 10;

const displayGroupSpanMap = computed(() => {
  const spanMap: Record<number, number> = {};
  const rows = props.list || [];

  for (let index = 0; index < rows.length; index += 1) {
    if (spanMap[index] !== undefined) continue;

    const currentGroup = String(rows[index]?.display_group || '-');
    let rowspan = 1;

    for (let nextIndex = index + 1; nextIndex < rows.length; nextIndex += 1) {
      const nextGroup = String(rows[nextIndex]?.display_group || '-');
      if (nextGroup !== currentGroup) break;
      rowspan += 1;
      spanMap[nextIndex] = 0;
    }

    spanMap[index] = rowspan;
  }

  return spanMap;
});

function handleSpanMethod({ columnIndex, rowIndex }: { columnIndex: number; rowIndex: number }) {
  if (columnIndex !== DISPLAY_GROUP_COLUMN_INDEX) {
    return { colspan: 1, rowspan: 1 };
  }

  const rowspan = displayGroupSpanMap.value[rowIndex] ?? 1;
  return rowspan === 0
    ? { colspan: 0, rowspan: 0 }
    : { colspan: 1, rowspan };
}
...
function getRowIndex(index: number) {
  const pageNo = Number(props.pageNo) || 1;
...
<template>
  <el-card shadow="never" class="table-card">
    ...
    <div class="table-query-bar">
      <el-form inline>
        ...
        <el-form-item label="项目分类">
          <el-select v-model="queryForm.item_category" placeholder="全部" clearable style="width: 150px">
            <el-option v-for="item in categoryOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="项目方向">
          <el-select v-model="queryForm.item_direction" placeholder="全部" clearable style="width: 150px">
            <el-option v-for="item in directionOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="录入模式">
          <el-select v-model="queryForm.input_mode" placeholder="全部" clearable style="width: 150px">
            <el-option v-for="item in inputModeOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="状态">
          <el-select v-model="queryForm.is_enabled" placeholder="全部" clearable style="width: 130px">
    ...
    <el-table v-loading="loading" :data="list" border stripe height="520" class="payroll-table" :span-method="handleSpanMethod">
      ...
      <el-table-column label="显示分组" min-width="130" class-name="display-group-cell" show-overflow-tooltip>
    ...
    <div class="pager-wrap">
      <el-pagination background layout="total, sizes, prev, pager, next, jumper" :total="total" :current-page="pageNo"
        :page-size="pageSize" :page-sizes="[10, 20, 50, 100]" @size-change="(size) => emit('sizeChange', size)"
...
.table-query-bar :deep(.el-form-item) {
  margin-bottom: 8px;
}
...
.payroll-table :deep(.display-group-cell) {
  background: #f8fafc;
  color: #1f2937;
  font-weight: 600;
}
