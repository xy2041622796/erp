...
export function usePayrollPage() {
  ...
  const sortedPayrollList = computed(() => {
    return [...(pageData.list || [])].sort((a, b) => {
      const groupA = String(a.display_group || '');
      const groupB = String(b.display_group || '');
      const groupCompare = groupA.localeCompare(groupB, 'zh-Hans-CN');
      ...
      return String(a.item_code || '').localeCompare(String(b.item_code || ''), 'zh-Hans-CN');
    });
  });
  const pagedList = computed(() => buildPagedList({ list: sortedPayrollList.value, total: pageData.total }, queryForm));
