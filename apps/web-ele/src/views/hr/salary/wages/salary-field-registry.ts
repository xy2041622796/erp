...
export function ensureStandardSalaryItemValues(row: Record<string, any>) {
  if (!row.itemValues) row.itemValues = {};
  const companySocial = Number(getSalaryFieldValue(row, 'company_social_insurance') || 0);
  const companyFund = Number(getSalaryFieldValue(row, 'company_housing_fund') || 0);
  setSalaryFieldValue(row, 'company_social_insurance', companySocial);
  setSalaryFieldValue(row, 'company_housing_fund', companyFund);
  setSalaryFieldValue(row, 'company_contribution_total', Number((companySocial + companyFund).toFixed(2)));
}
