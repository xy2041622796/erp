...
function roundMoneyByMode(value: number, roundMode: RoundMode = 'ROUND') {
...
export function calcSocialInsuranceByRule(rule: SocialInsuranceRule, itemValues: Record<string, number>) {
  ...
  const components: SocialInsuranceComponentAmount[] = (rule.components || []).map((component) => ({
    ...component,
    personalAmount: roundMoneyByMode(contributionBase * Number(component.personalRate || 0), rule.roundMode),
    companyAmount: roundMoneyByMode(contributionBase * Number(component.companyRate || 0), rule.roundMode),
  }));
  ...
  return {
    ...
    totalPersonalAmount: roundMoneyByMode(
      components.reduce((sum, item) => sum + Number(item.personalAmount || 0), 0),
      rule.roundMode,
    ),
    totalCompanyAmount: roundMoneyByMode(
      components.reduce((sum, item) => sum + Number(item.companyAmount || 0), 0),
      rule.roundMode,
    ),
  };
}
...
export function calcHousingFundByRule(rule: HousingFundRule, itemValues: Record<string, number>) {
  ...
  return {
    rawBaseAmount,
    contributionBase,
    personalAmount: roundMoneyByMode(contributionBase * Number(rule.personalRate || 0), rule.roundMode),
    companyAmount: roundMoneyByMode(contributionBase * Number(rule.companyRate || 0), rule.roundMode),
  };
}
...
export function calcSalaryTaxByRule(rule: SalaryTaxRule, itemValues: Record<string, number>) {
  ...
  const taxableIncome = Math.max(
    rule.taxableIncomeMode === 'already_taxable' ? rawBaseAmount : rawBaseAmount - Number(rule.threshold || 0),
  ...
  const taxAmount = roundMoneyByMode(
    taxableIncome * Number(matchedBracket.rate || 0) - Number(matchedBracket.quickDeduction || 0),
    rule.roundMode,
  );
