...
defineOptions({ name: 'HrAttendancePage' });
  ...
  <Page auto-content-height>
    <div class="hr-attendance-info">
      <div class="page-header">
        <div>
          <h2>考勤信息</h2>
          <p>与 siweiOA /hr/attendance -> /hr/attendance/info 跳转目标保持一致。</p>
        </div>
      ...
      <el-card class="query-card" shadow="never">
        <el-form :model="queryForm" inline @submit.prevent>
          <el-form-item label="关键词">
            <el-input v-model="queryForm.q" clearable placeholder="考勤编号/人员/部门/岗位" style="width: 260px" @keyup.enter="load" />
          </el-form-item>
          ...
          <el-form-item>
            ...
            <el-button @click="queryForm.q='';queryForm.status='all';queryForm.date='';load()">重置</el-button>
          </el-form-item>
        </el-form>
      </el-card>
      ...
      <el-card shadow="never">
        <el-table v-loading="loading" :data="rows" row-key="id" border height="100%">
          ...
          <el-table-column label="签到" min-width="170"><template #default="{ row = {}} = {}">{{ normalizeDateTime(row.check_in_at) }}</template></el-table-column>
          <el-table-column label="签退" min-width="170"><template #default="{ row = {}} = {}">{{ normalizeDateTime(row.check_out_at) }}</template></el-table-column>
          <el-table-column prop="work_minutes" label="工时(分钟)" width="110" />
          <el-table-column label="状态" width="100"><template #default="{ row = {}} = {}"><el-tag effect="plain">{{ row.status || '-' }}</el-tag></template></el-table-column>
      ...
      <el-dialog v-model="dialogOpen" :title="editing ? '编辑考勤' : '新增考勤'" width="720px" destroy-on-close>
        <el-form label-width="120px" :model="form">
          <el-row :gutter="16">
            <el-col :span="12"><el-form-item label="考勤编号"><el-input v-model="form.attendanceCode" disabled placeholder="保存后自动生成" /></el-form-item></el-col>
            <el-col :span="12"><el-form-item label="考勤日期" required><el-date-picker v-model="form.attendanceDate" type="date" value-format="YYYY-MM-DD" style="width: 100%" /></el-form-item></el-col>
            <el-col :span="24"><el-form-item label="人员/部门/岗位" required><el-select v-model="form.userDjRowid" filterable remote reserve-keyword style="width: 100%" @focus="loadUserOptions"><el-option v-for="item in userDjOptions" :key="item.userDjRowid" :label="`${item.userName} / ${item.depName} / ${item.jobName}`" :value="item.userDjRowid" /></el-select></el-form-item></el-col>
            <el-col :span="12"><el-form-item label="状态"><el-select v-model="form.status" style="width: 100%"><el-option label="正常" value="正常" /><el-option label="迟到" value="迟到" /><el-option label="早退" value="早退" /><el-option label="旷工" value="旷工" /></el-select></el-form-item></el-col>
            <el-col :span="12"><el-form-item label="工作分钟数"><el-input-number v-model="form.workMinutes" :min="0" style="width: 100%" /></el-form-item></el-col>
            <el-col :span="12"><el-form-item label="签到时间"><el-date-picker v-model="form.checkInAt" type="datetime" value-format="YYYY-MM-DDTHH:mm" style="width: 100%" /></el-form-item></el-col>
            <el-col :span="12"><el-form-item label="签到方式"><el-input v-model="form.checkInType" /></el-form-item></el-col>
            <el-col :span="12"><el-form-item label="签退时间"><el-date-picker v-model="form.checkOutAt" type="datetime" value-format="YYYY-MM-DDTHH:mm" style="width: 100%" /></el-form-item></el-col>
            <el-col :span="12"><el-form-item label="签退方式"><el-input v-model="form.checkOutType" /></el-form-item></el-col>
