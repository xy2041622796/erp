...
function openEdit(row: HrAttendanceApi.AttendanceRecord) {
  ...
  const checkInAt = toDatetimeLocal(row.check_in_at);
  const checkOutAt = toDatetimeLocal(row.check_out_at);
  form.checkRange = checkInAt && checkOutAt ? [checkInAt, checkOutAt] : [];
...
async function submit() {
  ...
  if (!Array.isArray(form.checkRange) || form.checkRange.length !== 2) {
    ElMessage.warning('请选择签到/签退时间');
    return;
  }

  saving.value = true;
  try {
    const [checkInAt, checkOutAt] = form.checkRange;
      ...
      <el-dialog v-model="dialogOpen" :title="editing ? '编辑考勤' : '新增考勤'" width="720px" destroy-on-close>
        <el-form label-width="120px" :model="form">
          <el-row :gutter="16">
            ...
            <el-col :span="24">
              <el-form-item label="签到/签退时间" required>
                <el-date-picker
                  v-model="form.checkRange"
                  type="datetimerange"
                  range-separator="至"
