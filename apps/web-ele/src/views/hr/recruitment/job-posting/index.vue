...
async function load() {
  loading.value = true;
  try {
    const res = await listJobPostingsWithPerm({ q: queryForm.q.trim() || undefined, status: queryForm.status === 'all' ? undefined : queryForm.status });
...
async function submit() {
  ...
  if (form.salary_min === '') {
    ElMessage.warning('请输入最低薪资');
    return;
  }
  if (form.salary_max === '') {
    ElMessage.warning('请输入最高薪资');
    return;
  }
...
async function handleDelete(row: HrRecruitmentJobPostingApi.JobPosting) {
  if (!row.id) return;
  try {
    await ElMessageBox.confirm(`确认删除职位「${row.title || row.job_code || row.id}」吗？`, '删除确认', {
    ...
    <div class="hr-recruitment-job-posting">
      ...
      <el-card class="query-card" shadow="never">
        <el-form :model="queryForm" inline @submit.prevent>
          <el-form-item label="关键词">
            <el-input v-model="queryForm.q" clearable placeholder="职位编号 / 标题 / 部门 / 岗位" style="width: 320px" @keyup.enter="load" />
          </el-form-item>
      ...
      <el-card shadow="never">
        ...
        <el-table v-loading="loading" :data="rows" row-key="id" border height="100%">
          <el-table-column prop="job_code" label="职位编号" min-width="150" show-overflow-tooltip />
          ...
          <el-table-column label="薪资范围" min-width="140">
            <template #default="scope">{{ scope?.row && (scope.row.salary_min != null || scope.row.salary_max != null) ? `${scope.row.salary_min ?? '-'} ~ ${scope.row.salary_max ?? '-'}` : '-' }}</template>
          </el-table-column>
          <el-table-column label="状态" width="110">
            <template #default="scope"><el-tag :type="scope?.row?.status === 'open' ? 'success' : 'info'" effect="plain">{{ scope?.row?.status === 'open' ? '招聘中' : '已关闭' }}</el-tag></template>
          </el-table-column>
          <el-table-column label="操作" fixed="right" width="130">
            <template #default="scope">
              <el-button link type="primary" :disabled="!scope?.row" @click="scope?.row && openEdit(scope.row)">编辑</el-button>
              <el-button link type="danger" :disabled="!scope?.row" @click="scope?.row && handleDelete(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-card>

      <el-dialog v-model="dialogOpen" :title="editing ? '编辑职位' : '新增职位'" width="780px" destroy-on-close>
        <el-form :model="form" label-width="110px">
          <el-row :gutter="16">
            <el-col :span="12"><el-form-item label="职位编号"><el-input v-model="form.job_code" disabled placeholder="保存后自动生成" /></el-form-item></el-col>
            ...
            <el-col :span="24">
              <el-form-item label="部门 / 岗位" required>
                <el-select v-model="form.jobOptionKey" filterable placeholder="选择部门与岗位" style="width: 100%" @focus="loadJobOptions">
                  <el-option v-for="item in jobOptions" :key="jobOptionKey(item)" :label="`${item.depName} / ${item.jobName}`" :value="jobOptionKey(item)" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12"><el-form-item label="招聘人数"><el-input v-model="form.headcount" type="number" /></el-form-item></el-col>
            <el-col :span="12">
              <el-form-item label="状态">
                <el-select v-model="form.status" style="width: 100%"><el-option label="招聘中" value="open" /><el-option label="已关闭" value="closed" /></el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12"><el-form-item label="最低薪资" required><el-input v-model="form.salary_min" type="number" /></el-form-item></el-col>
            <el-col :span="12"><el-form-item label="最高薪资" required><el-input v-model="form.salary_max" type="number" /></el-form-item></el-col>
            <el-col :span="12"><el-form-item label="发布日期"><el-input v-model="form.published_at" type="date" /></el-form-item></el-col>
            <el-col :span="12"><el-form-item label="截止日期"><el-input v-model="form.deadline" type="date" /></el-form-item></el-col>
            <el-col :span="24">
              <el-form-item label="任职要求">
                <RichTextarea v-model="form.requirements" :height="260" :show-image-upload="false" />
              </el-form-item>
            </el-col>
            <el-col :span="24"><el-form-item label="岗位职责"><el-input v-model="form.responsibilities" :rows="3" type="textarea" /></el-form-item></el-col>
          </el-row>
        </el-form>
        <template #footer>
...
<style scoped>
.hr-recruitment-job-posting { display: flex; min-height: 100%; flex-direction: column; gap: 16px; }
