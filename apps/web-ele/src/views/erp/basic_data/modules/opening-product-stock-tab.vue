...
defineOptions({ name: 'ErpBasicDataOpeningProductStockTab' });
...
const totalQty = computed(() => rows.value.reduce((sum, item) => sum + Number(item.qty || 0), 0));
  ...
  <div class="opening-tab-page">
    ...
    >
      <template #default>
        <div class="text-sm leading-6">
          本页已接入商品与仓库基础数据联动。后续接入后端时，应在审核通过后统一写入 erp_stock，并同步生成 erp_stock_record。
        </div>
      </template>
    </ElAlert>

    <div class="toolbar">
      <div class="text-sm text-gray-500">入口：ERP / 基础数据 / 商品期初库存录入</div>
    ...
    <ElTable v-else v-loading="loading" :data="rows" border>
      ...
      <ElTableColumn label="商品" min-width="220">
        <template #default="{ row }">
          <ElSelect v-model="row.product_id" filterable clearable placeholder="请选择商品" class="w-full" @change="handleProductChange($event, row)">
            <ElOption
      ...
      <ElTableColumn label="数量" min-width="120">
        <template #default="{ row }">
          <ElInputNumber v-model="row.qty" :precision="3" :controls="false" class="w-full" @change="recalcRow(row)" />
        </template>
      </ElTableColumn>
      <ElTableColumn label="单价" min-width="120">
        <template #default="{ row }">
          <ElInputNumber v-model="row.price" :precision="2" :controls="false" class="w-full" @change="recalcRow(row)" />
        </template>
      </ElTableColumn>
