...
defineOptions({ name: 'ErpBasicDataProductService' });
...
const productCategoryMap = ref<
  Map<number | string, { id: string; name: string }>
>(new Map());

function getProductCategoryName(categoryId?: string) {
  const key = String(categoryId ?? '');
  if (!key) return '-';
  const hit = productCategoryMap.value.get(key);
  return hit?.name || key;
}

function getWarehouseName(row: ErpProductApi.Product) {
  const directName = String(
    (row as any)?.default_warehouse_name ??
      (row as any)?.defaultWarehouseName ??
  ...
  return hit?.name || hit?.warehouse_name || hit?.warehouseName || '-';
}
...
function normalizeProductImageUrl(url?: string) {
  ...
  try {
    ...
    const isInsecureAbsoluteUrl = parsed.protocol === 'http:';
    const isHttpsPage = window.location.protocol === 'https:';

    if (isHttpsPage && isInsecureAbsoluteUrl) {
...
async function handleExport() {
  ...
  downloadFileFromBlobPart({ fileName: '产品服务信息.xls', source: data });
}
...
onMounted(async () => {
  const [list, categoryMap] = await Promise.all([
    getWarehouseSimpleList({ includeDisabled: true }),
    getProductCategoryMap(),
  ]);
  warehouseList.value = Array.isArray(list) ? list : [];
  productCategoryMap.value = categoryMap;
});
</script>
...
<template>
  <Page auto-content-height>
    ...
    <Grid table-title="产品/服务列表">
          ...
          :actions="[
            {
              ...
              tooltip: { content: '创建新的产品/服务', placement: 'top' },
      ...
      <template #image="{ row }">
        <div style="display: flex; gap: 8px; align-items: center">
          <img
            ...
            :src="normalizeProductImageUrl(row.product_image)"
            alt=""
          ...
          <div
            :style="{
              display: row.product_image ? 'none' : 'flex',
            }"
            style="
              display: flex;
              align-items: center;
      ...
      <template #product_category_id="{ row }">
        {{ getProductCategoryName(row.product_category_id) }}
      </template>
    </Grid>
  </Page>
</template>
