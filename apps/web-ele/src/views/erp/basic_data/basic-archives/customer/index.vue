...
async function handleExport() {
  ...
  downloadFileFromBlobPart({ fileName: '客户档案.xls', source: data });
}

function handleCreate() {
  currentCustomer.value = { companyType: 1 } as CrmCustomerApi.Customer;
  currentReadonly.value = false;
  dialogVisible.value = true;
}

function handleEdit(row: CrmCustomerApi.Customer) {
  currentCustomer.value = { ...row, companyType: 1 } as CrmCustomerApi.Customer;
  currentReadonly.value = false;
  dialogVisible.value = true;
}
...
<template>
  <Page auto-content-height>
      ...
      <template #toolbar-tools>
        <TableAction
          :actions="[
            {
              label: '客户',
              tooltip: { content: '新增客户', placement: 'top' },
              type: 'primary',
            ...
            {
              label: '导出数据',
              tooltip: { content: '导出客户数据为Excel文件', placement: 'top' },
              type: 'primary',
    ...
    <ElDialog
      ...
      :title="
        currentCustomer?.id || currentCustomer?.rowid ? '编辑客户' : '新增客户'
      "
      width="70%"
      top="5vh"
