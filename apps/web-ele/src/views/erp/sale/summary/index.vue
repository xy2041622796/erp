...
import type { ErpSaleSummaryApi } from '#/api/erp/sale/summary';
...
import { getSaleSummaryPage } from '#/api/erp/sale/summary';
import { getWarehouseSimpleList } from '#/api/erp/stock/warehouse';
import { getSimpleUserList } from '#/api/system/user';

import { useGridColumns, useGridFormSchema } from './data';

defineOptions({ name: 'ErpSaleSummary' });
...
const summaryRow = ref<ErpSaleSummaryApi.SaleSummaryRow>();
...
function getCustomerById(customerId?: string | number) {
  const id = normalizeId(customerId);
  return customerList.value.find((item) => {
    const keys = [item.id, item.rowid, item.ROWID, item.customer_id, item.customer_code, item.code];
    return keys.some((key) => normalizeId(key) === id);
  });
}
...
function getCustomerDisplayName(row: ErpSaleSummaryApi.SaleSummaryRow) {
  const customer = getCustomerById(row.customer_id);
  return pickText(customer, ['name', 'customer_name', 'Name', 'label']) || row.customer_name || '';
}

function getSellerDisplayName(row: ErpSaleSummaryApi.SaleSummaryRow) {
  const user = getUserById(row.seller_id);
  return pickText(user, ['UserName', 'userName', 'name', 'RealName', 'real_name', 'NickName', 'label']) || row.seller_name || '';
}


function getProductDisplayName(row: ErpSaleSummaryApi.SaleSummaryRow) {
...
function getProductCode(row: ErpSaleSummaryApi.SaleSummaryRow) {
  const product = getProductById(row.product_id);
  return row.product_code || pickText(product, ['product_code', 'code', 'product_no', 'no', 'bar_code', 'product_bar_code']) || '';
}

function getProductCategoryName(row: ErpSaleSummaryApi.SaleSummaryRow) {
  const product = getProductById(row.product_id);
...
function getRowClassName({ row }: { row: ErpSaleSummaryApi.SaleSummaryRow }) {
  return row.row_type !== 'data' ? 'sale-summary-subtotal-row' : '';
}
...
function footerMethod({ columns }: { columns: any[] }) {
  ...
  return [
    columns.map((column) => {
      ...
      if (["out_count","return_count","net_count"].includes(field)) return erpNumberFormatter((summary as any)[field], 3);
      if (["out_amount","return_amount","net_amount"].includes(field)) return erpNumberFormatter((summary as any)[field], 2);
      return '';
    }),
  ];
}
...
onMounted(async () => {
  const [customers, warehouses, users, products, categories] = await Promise.all([
...
const [Grid, gridApi] = useVbenVxeGrid({
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues) => {
          const res = await getSaleSummaryPage({
  ...
  } as VxeTableGridOptions<ErpSaleSummaryApi.SaleSummaryRow>,
});
</script>

<template>
  <Page auto-content-height>
    <Grid table-title="销售汇总">
      ...
      <template #customer_id="{ row }">
        ...
        <span v-else>{{ getCustomerDisplayName(row) }}</span>
      </template>

      <template #seller_id="{ row }">
        <span v-if="row.row_type !== 'data'"> </span>
        <span v-else>{{ getSellerDisplayName(row) }}</span>
      </template>
...
<style scoped>
:deep(.sale-summary-subtotal-row) {
  font-weight: 600;
