...
import {
  getSaleReturnInSource,
  getSaleReturnInSourcePage,
...
const groupOptions = computed(() => {
  ...
  for (const item of items) {
    ...
    if (!wid) continue;
    const remain = Number(item?.count || 0) - Number(item?.in_count || 0);
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues) =>
          await getSaleReturnInSourcePage({
...
function emitSelectedGroup() {
  ...
  const items = (fullReturn.value?.items || [])
    ...
    .map((item: any) => ({
      ...
      count: Math.max(
        Number(item?.count || 0) - Number(item?.in_count || 0),
        0,
      ),
...
async function handleOk() {
  if (!returnDoc.value) {
    ElMessage.warning('请选择一张销售退货单');
    return;
  }
  const detail = await getSaleReturnInSource(
        ...
        .filter((wid: string) =>
          items.some(
            (item: any) =>
              ...
              Number(item?.count || 0) - Number(item?.in_count || 0) > 0,
          ),
        ),
    ),
  ];
  if (groupedWarehouseIds.length === 0) {
    ElMessage.warning('该销售退货单没有可执行的剩余入库明细');
    return;
  }
  ...
  <div>
    ...
    >
      ...
      <div class="mb-3 text-sm text-[#666]">
        当前来源退货单中的产品分布在多个仓库，请选择本次要执行入库的仓库，系统将只导入该仓库下的产品。
      </div>
      <ElRadioGroup
