  ...
  <Modal :title="getTitle" class="!w-[80vw]" :footer="false">
    <div class="px-6 pb-4">
      ...
      <Form>
        ...
        <template #project_id><div class="w-full"><ElInput :model-value="selectedProjectName" readonly placeholder="请选择项目" class="!w-full" :disabled="!currentCustomerId" @click="openProjectSelect"><template #append><ElButton :disabled="!currentCustomerId" @click="openProjectSelect">查询</ElButton></template></ElInput></div></template>
      ...
      <div class="mt-6 flex items-center gap-3"><template v-if="!isDetailMode"><ElButton type="primary" :loading="saving" @click="handleSave(true)">保存并提交</ElButton><ElButton :loading="saving" @click="handleSave(false)">保存草稿</ElButton></template><ElButton :disabled="saving" @click="handleCancel">{{ isDetailMode ? '关闭' : '取消' }}</ElButton></div>
    </div>
  </Modal>
</template>
