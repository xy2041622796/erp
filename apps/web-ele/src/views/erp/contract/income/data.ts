...
export function useFormSchema(): VbenFormSchema[] {
  ...
  return [
    ...
    },
    {
      ...
      dependencies: {
        triggerFields: ['contract_amount', 'contract_tax_rate', 'taxIncluded'],
        trigger(values, form) {
          const amount = Number(values.contract_amount ?? 0);
          const rate = Number(values.contract_tax_rate ?? 0) / 100;
          const taxIncluded = Number(values.taxIncluded ?? 1) === 1;
          const taxAmount = erpPriceMultiply(amount, rate) ?? 0;
          const total = taxIncluded ? amount : amount + taxAmount;
          form.setFieldValue('contract_tax_amount', taxAmount);
          form.setFieldValue('contract_total_amount', total);
        },
      },
    },
    {
