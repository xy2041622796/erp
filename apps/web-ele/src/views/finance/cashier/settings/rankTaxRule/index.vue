...
const filteredRows = computed(() => {
  ...
  return rows.value.filter((row) => {
    ...
    const matchedStatus =
      ...
      (bindStatus.value === 'bound' && !!row.rule_code) ||
      (bindStatus.value === 'enabled' && !!row.rule_code && Number(row.is_enabled ?? 1) === 1) ||
      (bindStatus.value === 'unbound' && !row.rule_code);
  ...
  <Page auto-content-height>
    <div class="page-wrap">
      <el-card shadow="never">
        ...
        <div class="query-row">
          <el-input v-model="keyword" clearable placeholder="搜索职级编码 / 职级名称 / 个税规则 / 备注" style="width: 320px" />
