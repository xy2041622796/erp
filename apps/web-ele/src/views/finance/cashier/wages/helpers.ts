...
import { getCanonicalSalaryFieldCode, getSalaryFieldValue } from '#/views/finance/cashier/wages/salary-field-registry';
import { calcSalaryTaxByRule, type SalaryTaxRule } from '#/views/finance/cashier/wages/tax-rules';
...
function calcImportRowTotals(itemValues: Record<string, number>, itemMetaMap: Map<string, any>) {
  ...
  Object.entries(itemValues).forEach(([itemCode, itemValue]) => {
    ...
    if (itemDirection === 'income') {
      shouldPay = moneyNumber(addMoney([shouldPay, money]));
    }
    if (itemDirection === 'deduct') {
      deductTotal = moneyNumber(addMoney([deductTotal, money]));
    }
    if (itemCategory === 'TAX' || itemCode === 'personal_income_tax') {
      tax = moneyNumber(addMoney([tax, money]));
    }
  });
  ...
  const realPay = normalizeNumber(subMoney(subMoney(shouldPay, deductTotal, 'round', 6), tax));
...
function parseWorksheetRowsByScheme(
  ...
  for (let rowNumber = dataStartRow; rowNumber <= maxRowNumber; rowNumber += 1) {
    ...
    const finalRealPay = importedSummaryFields.has('realPay')
      ...
      : normalizeNumber(subMoney(subMoney(finalShouldPay, finalDeductTotal, 'round', 6), finalTax));
    ...
    const ruleCalculatedRealPay = normalizeNumber(subMoney(subMoney(finalShouldPay, finalDeductTotal, 'round', 6), ruleCalculatedTax));
