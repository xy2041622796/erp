...
import { WAGES_OVERALL_SCHEME_ID } from '#/views/finance/cashier/wages/helpers';
import {
  ...
  <el-dialog v-model="visible" width="92%" top="4vh" destroy-on-close append-to-body :z-index="1200"
    ...
    <div class="content" v-loading="loading">
      <el-descriptions :column="3" border class="desc-card">
        ...
        <el-descriptions-item label="应发工资合计">{{ formatMoney(header.total_should_pay) }}</el-descriptions-item>
        <el-descriptions-item label="实发工资">{{ formatMoney(header.total_actual_pay) }}</el-descriptions-item>
        <el-descriptions-item label="工资税费合计">{{ formatMoney(header.total_tax) }}</el-descriptions-item>
        <el-descriptions-item label="已发工资">{{ formatMoney(header.paid_salary) }}</el-descriptions-item>
        <el-descriptions-item label="核销金额">{{ formatMoney(header.write_off_amount) }}</el-descriptions-item>
        <el-descriptions-item label="应发余额">{{ formatMoney(header.remaining_should_pay)
      ...
      <el-table :data="detailRows" border height="460" class="wage-table">
        ...
        <el-table-column v-for="group in groupedDynamicColumns" :key="group.key" :label="group.label" align="center">
          <el-table-column v-for="item in group.items" :key="item.item_code" :label="getColumnTitle(item)"
            ...
            <template #default="{ row }">
              <span class="money">{{ formatMoney(getItemValue(row, String(item.item_code || ''))) }}</span>
            </template>
          </el-table-column>
        </el-table-column>
        ...
        <el-table-column label="公司承担" align="center">
          <el-table-column label="公司社保" width="120" align="right">
            <template #default="{ row }">
              <span class="money">{{ formatMoney(row.companySocial) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="公司公积金" width="130" align="right">
            <template #default="{ row }">
              <span class="money">{{ formatMoney(row.companyFund) }}</span>
            </template>
          </el-table-column>
        </el-table-column>
        ...
        <el-table-column label="汇总结果" align="center" fixed="right">
          <el-table-column label="应发合计" width="120" align="right">
            <template #default="{ row }">
              <span class="money">{{ formatMoney(row.shouldPay) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="扣减合计" width="120" align="right">
            <template #default="{ row }">
              <span class="money">{{ formatMoney(row.deductTotal) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="个税合计" width="120" align="right">
            <template #default="{ row }">
              <span class="money">{{ formatMoney(row.taxValue) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="实发工资" width="120" align="right">
            <template #default="{ row }">
              <span class="money">{{ formatMoney(row.realPay) }}</span>
            </template>
          </el-table-column>
        </el-table-column>
