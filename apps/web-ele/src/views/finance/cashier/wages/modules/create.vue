...
import { evaluateFormulaExpr } from '#/views/finance/cashier/settings/payrollFormula/helpers';
...
} from '#/views/finance/cashier/wages/salary-field-registry';
...
const totalShouldPay = computed(() => moneyNumber(sumByMoney(rows.value, (row) => calcShouldPay(row))));
const totalTax = computed(() => moneyNumber(sumByMoney(rows.value, (row) => calcTax(row))));
const totalRealPay = computed(() => moneyNumber(sumByMoney(rows.value, (row) => calcRealPay(row))));
...
function applySocialInsuranceRules(row: EmployeeSalaryRow) {
  ...
  salaryItems.value.forEach((item) => {
    ...
    matchedRules.forEach((rule) => {
      ...
      itemAmount = moneyNumber(addMoney([itemAmount, result.amount]));
      const ruleCode = normalizeText(rule.code);
      if (ruleCode && !countedRuleCodes.has(ruleCode)) {
        companyTotal = moneyNumber(addMoney([companyTotal, result.calcResult?.totalCompanyAmount]));
...
function applyHousingFundRules(row: EmployeeSalaryRow) {
  ...
  salaryItems.value.forEach((item) => {
    ...
    matchedRules.forEach((rule) => {
      ...
      personalAmount = moneyNumber(addMoney([personalAmount, result.personalAmount]));
      companyAmount = moneyNumber(addMoney([companyAmount, result.companyAmount]));
      const ruleCode = normalizeText(rule.code);
      if (ruleCode && !countedRuleCodes.has(ruleCode)) {
        companyTotal = moneyNumber(addMoney([companyTotal, result.companyAmount]));
        countedRuleCodes.add(ruleCode);
      }
      hasRule = true;
    });
...
function calcShouldPay(row: EmployeeSalaryRow) {
  return roundMoney(
    salaryItems.value
      .filter((item) => item.item_direction === 'income')
      .reduce((sum, item) => moneyNumber(addMoney([sum, getItemValue(row, String(item.item_code || ''))])), 0),
  );
}
  ...
  return roundMoney(
    salaryItems.value
      ...
      .reduce((sum, item) => {
        ...
        return moneyNumber(addMoney([sum, getItemValue(row, String(item.item_code || ''))]));
      }, 0),
  );
}

function calcRealPay(row: EmployeeSalaryRow) {
  return roundMoney(subMoney(calcShouldPay(row), calcDeduct(row)));
}
      ...
      <div class="table-wrap">
        <el-table :data="rows" border height="420" class="wage-table">
            ...
            <el-table-column v-for="item in group.items" :key="item.item_code" :label="getColumnTitle(item)"
              ...
              <template #default="{ row }">
                <div v-if="isFormulaReadonly(item)" class="readonly-cell">
                  {{ moneyText(getItemValue(row, String(item.item_code || ''))) }}
                </div>
          ...
          <el-table-column label="公司承担" align="center">
            <el-table-column label="公司社保" width="120" align="right">
              <template #default="{ row }">
                <div v-if="hasSocialRuleDrivenItem" class="readonly-cell">{{ moneyText(row.companySocial)
                  }}</div>
            ...
            <el-table-column label="公司公积金" width="130" align="right">
              <template #default="{ row }">
                <div v-if="hasHousingFundRuleDrivenItem" class="readonly-cell">{{ moneyText(row.companyFund) }}</div>
          ...
          <el-table-column label="汇总结果" align="center" fixed="right">
            <el-table-column label="应发合计" width="120" align="right">
              <template #default="{ row }">
                <span class="money">{{ moneyText(calcShouldPay(row)) }}</span>
              </template>
            </el-table-column>
            <el-table-column label="个税合计" width="120" align="right">
              <template #default="{ row }">
                <span class="money">{{ moneyText(calcTax(row)) }}</span>
              </template>
            </el-table-column>
            <el-table-column label="实发工资" width="120" align="right">
              <template #default="{ row }">
                <span class="money">{{ moneyText(calcRealPay(row)) }}</span>
              </template>
            </el-table-column>
          </el-table-column>
        ...
        <div class="table-bottom">
          ...
          <div class="stats">
            ...
            <div class="stats-row"><span>应发合计</span><strong>{{ moneyText(totalShouldPay) }}</strong></div>
            <div class="stats-row"><span>个税合计</span><strong>{{ moneyText(totalTax) }}</strong></div>
            <div class="stats-row"><span>实发合计</span><strong>{{ moneyText(totalRealPay) }}</strong></div>
          </div>
        </div>
      </div>
