...
function calcWageTotals(list: any[]) {
  totals.value = {
    shouldPayTotal: moneyNumber(sumByMoney(list, (item) => item.total_should_pay)),
    actualPayTotal: moneyNumber(sumByMoney(list, (item) => item.total_actual_pay)),
    paidTotal: moneyNumber(sumByMoney(list, (item) => item.paid_salary)),
    writeoffTotal: moneyNumber(sumByMoney(list, (item) => item.write_off_amount)),
    balanceTotal: moneyNumber(sumByMoney(list, (item) => item.remaining_should_pay)),
    taxTotal: moneyNumber(sumByMoney(list, (item) => item.total_tax)),
  };
}
...
function buildImportSummary(result: ParseWagesImportBySchemeIdResult): ImportSummary {
  ...
  return {
    ...
    deductTotal: moneyNumber(sumByMoney(rows, (row) => row.deductTotal)),
    ...
    realPayTotal: moneyNumber(sumByMoney(rows, (row) => row.realPay)),
    ...
    shouldPayTotal: moneyNumber(sumByMoney(rows, (row) => row.shouldPay)),
    taxTotal: moneyNumber(sumByMoney(rows, (row) => row.tax)),
  };
}
...
async function handleSettleCurrentMonth() {
  ...
  try {
    ...
    lastImportSummary.value = {
      attachmentName: '-',
      deductTotal: moneyNumber(sumByMoney(payload.rows, (row: any) => row.deductTotal)),
      dept: normalizeText(payload.dept),
      month: normalizeText(payload.month),
      payMonth: normalizeText(payload.payMonth),
      realPayTotal: moneyNumber(sumByMoney(payload.rows, (row: any) => row.realPay)),
      rowCount: Number(payload.rows.length || 0),
      savedSlipNo: normalizeText(saveResult?.slip_no),
      schemeName: '系统月度汇总计算',
      sheetName: '自动结算',
      shouldPayTotal: moneyNumber(sumByMoney(payload.rows, (row: any) => row.shouldPay)),
      taxTotal: moneyNumber(sumByMoney(payload.rows, (row: any) => row.tax)),
    };
  ...
  try {
    ...
    await ElMessageBox.confirm(
      [
        ...
        `应发合计：${moneyText(summary.shouldPayTotal)}，扣减合计：${moneyText(summary.deductTotal)}`,
        `个税合计：${moneyText(summary.taxTotal)}，实发合计：${moneyText(summary.realPayTotal)}`,
        '确认将解析结果直接保存为工资表吗？',
      ].join('\n'),
      '导入确认',
      {
...
<template>
  <Page auto-content-height>
    ...
    <div class="page-head">
      ...
      <div class="filters">
        <el-date-picker v-model="month" type="month" placeholder="选择月份" value-format="YYYY-MM" style="width: 140px" @change="handleFilter" />
      ...
      <div class="actions">
        ...
        <FileUpload v-model="importFileValue" button-text="上传工资" :api="fakeImportUploadApi" :auto-upload="true" :disabled="importLoading"
          :file-size="20" :file-type="['xls', 'xlsx']" :is-show-tip="false" :limit="1" :show-file-list="false" @success="handleImportUploadSuccess" />
    ...
    <div v-if="lastImportSummary" class="import-summary-card">
      ...
      <div class="import-summary-card__content">
        ...
        <div>应发合计：{{ moneyText(lastImportSummary.shouldPayTotal) }}</div>
        <div>扣减合计：{{ moneyText(lastImportSummary.deductTotal) }}</div>
        <div>个税合计：{{ moneyText(lastImportSummary.taxTotal) }}</div>
        <div>实发合计：{{ moneyText(lastImportSummary.realPayTotal) }}</div>
    ...
    <div v-if="activeTab === 'wage'" class="summary">
      <div>应发工资合计：{{ moneyText(totals.shouldPayTotal) }}</div>
      <div>实发工资合计：{{ moneyText(totals.actualPayTotal) }}</div>
      <div>已发工资合计：{{ moneyText(totals.paidTotal) }}</div>
      <div>核销金额合计：{{ moneyText(totals.writeoffTotal) }}</div>
      <div>应发余额合计：{{ moneyText(totals.balanceTotal) }}</div>
      <div>工资税费合计：{{ moneyText(totals.taxTotal) }}</div>
    </div>
  </Page>
</template>
