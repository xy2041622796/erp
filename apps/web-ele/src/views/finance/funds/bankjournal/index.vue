...
import {
  deleteBankjournalRow,
  deleteBankjournalRows,
  fetchBankAccounts,
  fetchBankjournalList,
  fetchCounterparties,
  fetchIoTypes,
  linkBankjournalVoucher,
  unlinkBankjournalVoucher,
  saveBankjournalRow,
  saveBankjournalRows,
  type BankAccount,
  type BankjournalItem,
  type Counterparty,
  type IoType,
} from '#/api/erp/finance/funds/bankjournal';
...
import { buildBankjournalPrintHtml } from '#/views/finance/print-templates/bankjournal';
...
defineOptions({ name: 'FinanceFundsBankJournal' });
...
type DataRow = {
  __type: 'data';
  __tmpKey?: string;
  /** 手动插入行的前端插入排序值：用于保存回填后仍保持在插入位置，不被分组/编号排序重新归位 */
...
function getAccountOptionLabel(account: any) {
  ...
  return [bankName, accountName].filter(Boolean).join(' ') || getAccountRootName(account);
}
...
function compareSubjectCode(a: any, b: any) {
  const ac = String(a?.subject_number || '').trim();
  const bc = String(b?.subject_number || '').trim();
  if (ac === bc) return 0;
  // 科目编码是层级编码，不能使用 numeric 自然排序；字符串顺序才能保证 1002001 跟在 1002 下面、1012001 跟在 1012 下面。
  return ac < bc ? -1 : 1;
}

const accountOptions = computed<BankAccount[]>(() => {
...
const bankSubjectOptions = computed(() => {
  return [...(subjects.value || [])]
    .filter((item: any) => {
      ...
      return (
        (code.startsWith('1002') || code.startsWith('1012')) &&
        Number(item?.lingma_sys_is_delete ?? 0) !== 1 &&
        String(item?.subject_state ?? '1') !== '0'
      );
    })
    .sort(compareSubjectCode);
});

function getBankSubjectCode(subject: any) {
  return String(subject?.subject_number || '').trim();
}

function getBankSubjectLevel(subject: any) {
  return getSubjectLevelByCode(getBankSubjectCode(subject));
}

function getBankSubjectIndent(subject: any) {
  return Math.max(0, getBankSubjectLevel(subject) - 1) * 18;
}

function isBankSubjectParentOption(subject: any) {
  const code = getBankSubjectCode(subject);
  if (!code) return false;
  return bankSubjectOptions.value.some((item: any) => {
    const itemCode = getBankSubjectCode(item);
    return itemCode.length > code.length && itemCode.startsWith(code);
  });
}

function getSubjectOptionLabel(subject: any) {
  const code = getBankSubjectCode(subject);
  const name = String(subject?.subject_name || '').trim();
  return [code, name].filter(Boolean).join(' ');
}

function syncAddAccountSubject(subjectId: string) {
  const subject = bankSubjectOptions.value.find((item: any) => String(item?.rowid || '') === String(subjectId || '')) as any;
  if (subject && isBankSubjectParentOption(subject)) {
    addAccountForm.subject_id = '';
    addAccountForm.subject_code = '';
    addAccountForm.subject_name = '';
    return;
  }
  addAccountForm.subject_id = String(subject?.rowid || '').trim();
  addAccountForm.subject_code = getBankSubjectCode(subject);
  addAccountForm.subject_name = String(subject?.subject_name || '').trim();
}

async function ensureBankSubjectsLoaded() {
  if (subjects.value.length > 0) return;
  const res = await getAllSubjectList({ pageNo: 1, page: 0, subject_state: 1, lingma_sys_is_delete: 0 } as any);
  subjects.value = (res?.list || []) as BilSubjectApi.Subject[];
}
...
function getCurrentBankAccount() {
  return (accounts.value || []).find(
    (item: any) => String(item?.id || item?.rowid || '') === String(query.accountId || ''),
  ) as any;
}

function getBankAccountSubjectText(account: any) {
  return String(account?.subjectId || account?.subject_id || account?.subjectCode || account?.subject_code || '').trim();
}

const accountDisplayName = computed(() => {
  const account = getCurrentBankAccount();
...
async function onAddAccount() {
  resetAddAccountForm();
  try {
    await ensureBankSubjectsLoaded();
  } catch (error: any) {
    console.error(error);
...
async function saveAddAccount() {
  if (!String(addAccountForm.bank_name || '').trim()) {
  ...
  if (!String(addAccountForm.subject_id || '').trim()) {
  ...
  try {
    ...
    const saved = await saveFundsAccount({
      ...
      bank_name: String(addAccountForm.bank_name || '').trim(),
      bank_account_no: String(addAccountForm.bank_account_no || '').trim(),
      subject_id: String(addAccountForm.subject_id || '').trim(),
      subject_code: String(addAccountForm.subject_code || '').trim(),
      subject_name: String(addAccountForm.subject_name || '').trim(),
      currency_code: 'CNY',
      currency_name: '人民币',
      enable_status: 1,
      union_bind_status: 0,
    ...
    accounts.value = await fetchBankAccounts({ showDisabled: showDisabledAccounts.value });
...
function getSelectedIoTypeByRow(r: DataRow) {
  return ioTypes.value.find((item) => String(item.id) === String(r.ioType || ''));
}

function getRowIoTypeCategoryType(r: DataRow) {
  const selectedIoType = getSelectedIoTypeByRow(r);
  return getIoTypeCategoryType(selectedIoType);
}

function hasIoTypeSubject(item: any) {
  return Boolean(String(item?.subjectId || item?.subject_id || item?.subjectCode || item?.subject_code || '').trim());
}
...
function getRowRequiredMissingFields(r: DataRow) {
  ...
  if (!getBankAccountSubjectText(getCurrentBankAccount())) missing.push('银行账户科目');
...
function getRowAmountGroup(row: DataRow) {
  if (moneyNumber(row.income || 0) !== 0) return 'income';
  if (moneyNumber(row.expense || 0) !== 0) return 'expense';
  const selectedIoType = ioTypes.value.find((item) => String(item.id) === String(row.ioType || ''));
  const categoryType = Number((selectedIoType as any)?.categoryType || 0);
  if (categoryType === 1) return 'income';
  if (categoryType === 2) return 'expense';
  return 'unknown';
}

function getRowAmountGroupIndex(row: DataRow) {
  const group = getRowAmountGroup(row);
  if (group === 'income') return 1;
  if (group === 'expense') return 2;
  return 3;
}
...
function getJournalNoSortValue(row: DataRow) {
  const text = String(row?.journalNo || '').trim();
  const match = text.match(/(\d+)$/);
  if (match) return Number(match[1]);
  return Number.MAX_SAFE_INTEGER;
}

function compareJournalBusinessOrder(a: DataRow, b: DataRow) {
  const dateDiff = String(a?.date || '').localeCompare(String(b?.date || ''));
  if (dateDiff !== 0) return dateDiff;

  const noDiff = getJournalNoSortValue(a) - getJournalNoSortValue(b);
  if (noDiff !== 0) return noDiff;

  const noTextDiff = String(a?.journalNo || '').localeCompare(
    String(b?.journalNo || ''),
    'zh-Hans-CN-u-kn-true',
    { numeric: true, sensitivity: 'base' },
  );
  if (noTextDiff !== 0) return noTextDiff;

  return String(a.id || a.__tmpKey || '').localeCompare(String(b.id || b.__tmpKey || ''));
}
...
function applyPrevNextInsertSortOrders(normalRows: DataRow[], manualRows: DataRow[]) {
  ...
  for (const group of groups.values()) {
    group.sort((a, b) => {
      ...
      return compareJournalBusinessOrder(a, b);
    });
...
function normalizeRowSortOrders() {
  // 普通数据行先按前端业务规则定序，并重新生成基准 sortOrder；这样不依赖接口 sort，也不受接口返回顺序影响。
  const normalRows = [...rows.value]
    .filter((item) => !isManualInsertedRow(item))
    .sort(compareJournalBusinessOrder);
  ...
  const sorted = [...normalRows, ...manualRows].sort((a, b) => {
    // 手动插入行优先根据 prev/next 锚点换算出的 sortOrder 排序，保证 A-B 之间插入后仍留在 A-B 之间。
...
function onMoneyChange(r: DataRow, field: 'income' | 'expense') {
  ...
  // 不再按收入/支出做大分类，输入金额时保持当前行位置，只重算余额。
  recalcBalance();
}
...
function onCounterpartyChange(r: DataRow) {
  const selectedCounterparty = counterparties.value.find((item) => String(item.id) === String(r.counterparty || ''));
...
async function saveOpeningBalance() {
  ...
  if (!accountId) { ElMessage.warning('请先选择银行账户'); return; }
  const account = (accounts.value || []).find((item: any) => String(item?.id || item?.rowid || '') === accountId) as any;
  if (!account) { ElMessage.warning('未找到当前银行账户'); return; }
  ...
  try {
    ...
    if (idx >= 0) accounts.value[idx] = { ...(accounts.value[idx] as any), initialAmount, initial_amount: initialAmount } as any;
    recalcBalance();
...
async function reload() {
  ...
  try {
    accounts.value = await fetchBankAccounts({ showDisabled: showDisabledAccounts.value });
    const res = await fetchBankjournalList({
...
async function saveRow(r: DataRow, options: { silent?: boolean; reloadAfterSave?: boolean } = {}) {
  ...
  try {
    ...
    const saved = await saveBankjournalRow({
      accountRowid: query.accountId,
      forceAdd: isNewRow,
      row: {
        ...r,
        id: isNewRow ? '' : r.id,
...
async function onPrintList() {
  ...
  const html = buildBankjournalPrintHtml({
    title: '银行日记账',
    companyName: String(
...
async function saveBatchRows(rowsToSave: DataRow[], successText: string) {
  ...
  await saveBankjournalRows(savedRows);
  exitEdit();
  await reload();
...
async function batchDeleteRows() {
  ...
  await ElMessageBox.confirm('确认批量删除所选 ' + rowsToDelete.length + ' 条银行日记账明细吗？删除后列表中将不再显示。', '批量删除', {
    type: 'warning',
    confirmButtonText: '确认删除',
    cancelButtonText: '取消',
  });
  await deleteBankjournalRows(rowsToDelete);
  selected.value = [];
  exitEdit();
  await reload();
...
async function batchClearVoucherLink() {
  const rowsToUpdate = getSelectedDataRows('取消关联');
  if (rowsToUpdate.length === 0) return;
  ...
  await ElMessageBox.confirm('将删除所选 ' + linkedRows.length + ' 条日记账的关联凭证信息，不删除原凭证，是否继续？', '取消关联', {
    type: 'warning',
    confirmButtonText: '删除关联',
    cancelButtonText: '取消',
  });
  await unlinkBankjournalVoucher(linkedRows);
  ...
  exitEdit();
  await reload();
  ElMessage.success('取消关联成功，已处理 ' + linkedRows.length + ' 条');
}
...
async function batchAssignIoType() {
  ...
  rowsToUpdate.forEach((row) => {
    ...
    const categoryType = Number((option as any)?.categoryType || 0);
    if (categoryType === 1 && Number(row.income || 0) === 0 && Number(row.expense || 0) !== 0) {
      row.income = Number(row.expense || 0);
      row.expense = 0;
    }
    if (categoryType === 2 && Number(row.expense || 0) === 0 && Number(row.income || 0) !== 0) {
      row.expense = Number(row.income || 0);
      row.income = 0;
    }
  });
...
async function onUnlinkVoucher() {
  const rowsToUnlink = selected.value.filter((row: any) => String(row.voucherMainId || '').trim() || String(row.voucherNo || '').trim());
  if (selected.value.length === 0) {
    ElMessage.warning('请先勾选要取消关联的银行日记账行');
    return;
  }
  ...
  try {
    await ElMessageBox.confirm(
      '确认取消所选 ' + rowsToUnlink.length + ' 条银行日记账的凭证关联吗？不会删除原凭证。',
      '取消关联',
      {
        type: 'warning',
    ...
    await unlinkBankjournalVoucher(rowsToUnlink);
    rowsToUnlink.forEach((row) => {
      row.voucherMainId = '';
      row.voucherNo = '';
      row.linkStatus = 0;
    });
...
async function downloadWorkbook(fileName: string, rowsToExport: DataRow[], templateOnly = false) {
  ...
  const sheet = workbook.addWorksheet(safeSheetName('银行日记账'));
  sheet.columns = [
  ...
  const dataRows = templateOnly
    ? [{ 单据编号: '', 日期: getDefaultRowDate(), 币别: '人民币', 摘要: '示例：银行收支摘要', 收支类别: '收入', 往来单位: '', 项目: '', 部门: '', 收入借方: 0, 支出贷方: 0, 余额: '', 关联凭证: '', 往来单位账号: '', 往来单位开户行: '', 结算方式: '', 票据号: '', 备注: '', 交易流水号: '' }]
...
async function exportCurrentRows() {
  ...
  await downloadWorkbook('银行日记账-' + suffix + '.xlsx', rows.value);
}
...
async function exportAllRows() {
  ...
  try {
    ...
    await downloadWorkbook('银行日记账-全部.xlsx', allRows);
  } catch (e: any) {
    console.error(e);
...
function resolveIoTypeByText(text: string, income: number, expense: number) {
  ...
  const byName = ioTypes.value.find((item) => String(item.name || '').trim() === normalized);
  if (byName) return String(byName.id);
  if (normalized === '收入' || normalized.toLowerCase() === 'in') return 'in';
  if (normalized === '支出' || normalized.toLowerCase() === 'out') return 'out';
  const categoryType = income > 0 && expense <= 0 ? 1 : expense > 0 && income <= 0 ? 2 : income >= expense ? 1 : 2;
  return String(ioTypes.value.find((item) => Number((item as any).categoryType || 0) === categoryType)?.id || (categoryType === 1 ? 'in' : 'out'));
}
...
async function importWorkbookFile(file: File) {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    const importedRow = {
      ...
      summary,
      ioType,
      ioTypeName: ioTypes.value.find((item) => String(item.id) === String(ioType))?.name || (ioType === 'in' ? '收入' : '支出'),
      counterparty: cp.id,
  ...
  await ElMessageBox.confirm(
    '将导入 ' + importedRows.length + ' 条银行日记账记录到当前银行账户，是否继续？',
    '导入确认',
...
async function downloadImportTemplate() {
  await downloadWorkbook('银行日记账-导入模板.xlsx', [], true);
}
...
function matchSubjectByTexts(texts: unknown[], options: { subjectTypes?: string[]; codePrefixes?: string[] } = {}) {
  ...
  const candidates = (subjects.value || []).filter((subject: any) => {
    ...
    const code = subjectCodeOf(subject);
    if (options.codePrefixes?.length && !options.codePrefixes.some((prefix) => code.startsWith(prefix))) return false;
    return true;
  });
  let best: any = null;
  let bestScore = 0;
...
function getCurrentBankSubject() {
  const account = (accounts.value || []).find((item: any) => String(item?.id || item?.rowid || '') === String(query.accountId || '')) as any;
  ...
  const accountKind = String((account as any)?.account_kind || '').trim();
  const matched = matchSubjectByTexts(
    [accountName, accountCode, accountKind, '银行存款', '其他货币资金', '银行', '货币资金'],
    { subjectTypes: ['1'] },
  );
  if (matched.code && matched.name) return matched;
  return matchSubjectByTexts(['银行存款', '其他货币资金', '货币资金'], { subjectTypes: ['1'] });
}

function findIoType(row: DataRow) {
  return ioTypes.value.find((item) => String(item.id) === String(row.ioType || ''));
}

function getOppositeSubjectByCategory(row: DataRow) {
  const income = moneyNumber(row.income || 0);
...
function buildVoucherDetailsByBankRow(r: DataRow): ErpVoucherApi.VoucherDetail[] {
  const income = moneyNumber(r.income || 0);
  const expense = moneyNumber(r.expense || 0);
  const amount = income > 0 ? income : expense;
  const bankSubject = getCurrentBankSubject();
  const oppositeSubject = getOppositeSubjectByCategory(r);
  const summary = String(r.summary || (income > 0 ? '银行收入' : '银行支出')).trim();
  if (income > 0) {
    return [
      { account_code: bankSubject.code, account_name: bankSubject.name, abstract_content: summary, debit_amount: amount, credit_amount: 0, sort_no: 1 },
  ...
  return [
    ...
    { account_code: bankSubject.code, account_name: bankSubject.name, abstract_content: summary, debit_amount: 0, credit_amount: amount, sort_no: 2 },
  ];
}

async function generateVoucherForRows(targets: DataRow[]) {
  ...
  if (subjects.value.length === 0) {
    try {
      const subjectRes = await getAllSubjectList({ pageNo: 1, page: 0, lingma_sys_is_delete: 0 } as any);
      subjects.value = (subjectRes?.list || []) as BilSubjectApi.Subject[];
    } catch (error) {
      console.error(error);
      ElMessage.error('加载科目表失败，无法模糊匹配生成凭证科目');
      return;
    }
  }

  const invalidSubjectRow = todoTargets.find((row) => {
    const categorySubject = getOppositeSubjectByCategory(row);
    const bankSubject = getCurrentBankSubject();
    return !bankSubject.code || !bankSubject.name || !categorySubject.code || !categorySubject.name;
  });
  if (invalidSubjectRow) {
...
function onGenVoucherCommand(cmd: string | number | object) {
  ...
  const targets = command === 'genAll' ? rows.value : selected.value;
  if (targets.length === 0) { ElMessage.warning(command === 'genAll' ? '当前没有可生成凭证的数据' : '请先勾选要生成凭证的日记账行'); return; }
  generateVoucherForRows(targets);
}
...
onMounted(async () => {
  ...
  if (!query.accountId && accounts.value.length > 0) {
    query.accountId = String(accounts.value[0]?.id || '');
  }
  ...
  <Page auto-content-height class="h-full">
    ...
    <div class="journal-page flex h-full flex-col gap-3">
      <div class="print-header">
        <div class="print-title">银行日记账</div>
      ...
      <div class="screen-toolbar journal-toolbar">
        <div class="journal-toolbar__filters">
          <div class="journal-filter-item journal-filter-item--account">
            <span class="journal-filter-label">银行账户</span>
            <ElSelect v-model="query.accountId" filterable class="journal-account"
              popper-class="journal-account-select-popper" placeholder="选择银行账户" @change="reload">
        ...
        <div class="journal-toolbar__actions">
          ...
          <ElDropdown @command="onGenVoucherCommand">
            ...
            <template #dropdown>
              <ElDropdownMenu>
                <ElDropdownItem command="genSelected">生成所选</ElDropdownItem>
                <ElDropdownItem command="genAll">生成全部</ElDropdownItem>
              </ElDropdownMenu>
            </template>
          </ElDropdown>
        </div>
      ...
      <div class="journal-print-wrap min-h-0 flex-1">
        <ElTable v-loading="loading" :data="tableRows" border height="100%" :row-key="tableRowKey"
          ...
          <ElTableColumn v-if="query.showAll" prop="counterpartyAccountNo" label="往来单位账号" width="150">
            <template #default="{ row }">
              <span>{{ row.__type === 'data' ? ((row as any).counterpartyAccountNo || '') : '' }}</span>
            </template>
          </ElTableColumn>

          <ElTableColumn v-if="query.showAll" prop="counterpartyBankName" label="往来单位开户行" width="160">
            <template #default="{ row }">
    ...
    <ElDialog v-model="addAccountDialogVisible" title="新增银行账户" width="min(420px, 92vw)" destroy-on-close
      class="add-account-dialog">
      <div class="add-account-form">
        <div class="add-account-form__item">
          <span class="add-account-form__label add-account-form__label--required">银行</span>
          <ElInput v-model="addAccountForm.bank_name" placeholder="请输入银行名称" />
        </div>
        <div class="add-account-form__item">
        ...
        <div class="add-account-form__item">
          <span class="add-account-form__label">银行卡号</span>
          <ElInput v-model="addAccountForm.bank_account_no" placeholder="请输入银行卡号" />
        </div>
        <div class="add-account-form__item">
          <span class="add-account-form__label add-account-form__label--required">会计科目</span>
          <ElSelect v-model="addAccountForm.subject_id" filterable clearable placeholder="请选择银行存款/其他货币资金科目"
            @change="(value: any) => syncAddAccountSubject(String(value || ''))">
            <ElOption v-for="subject in bankSubjectOptions" :key="String(subject.rowid || '')"
              :label="getSubjectOptionLabel(subject)" :value="String(subject.rowid || '')"
              :disabled="isBankSubjectParentOption(subject)">
              <span class="bank-subject-option"
                :class="{ 'bank-subject-option--parent': isBankSubjectParentOption(subject) }"
                :style="{ paddingLeft: getBankSubjectIndent(subject) + 'px' }">
...
.journal-toolbar__filters {
  display: grid;
  min-width: 0;
  grid-template-columns: 220px 352px;
  align-items: center;
  gap: 8px;
}
...
@media (max-width: 1280px) {
  ...
  .journal-toolbar__filters {
    grid-template-columns: 210px 332px;
    gap: 6px 8px;
  }
...
:global(.bank-subject-option--parent) {
  font-weight: 600;
  color: var(--el-text-color-primary);
}
