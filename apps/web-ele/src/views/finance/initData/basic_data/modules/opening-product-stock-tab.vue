...
function recalcRow(row: ProductOpeningRow) {
  row.amount = moneyNumber(mulMoney(row.qty, row.price));
}
...
const totalQty = computed(() =>
  sumByMoney(rows.value, (item) => item.qty, 'round', 3),
);
const totalAmount = computed(() => moneyNumber(sumByMoney(tableData.value || [], (item: any) => item.amount)));
