...
import { useFormItemColumns } from '#/views/finance/revenue/settlement/data';

interface Props {
  items?: ErpIncomeSettlementApi.IncomeSettlementItem[];
  disabled?: boolean;
  locked?: boolean;
}
...
const emit = defineEmits<{
  (e: 'update:items', v: ErpIncomeSettlementApi.IncomeSettlementItem[]): void;
  (e: 'update:summary', v: { amount: number; taxAmount: number; total: number }): void;
  (e: 'update:product-names', v: string): void;
}>();

const tableData = ref<ErpIncomeSettlementApi.IncomeSettlementItem[]>([]);
...
<template>
  <Grid class="w-full">
    ...
    <template #total_tax_price="{ row }">
      <span>{{ formatAmount(row.total_tax_price) }}</span>
    </template>

    <template #total_price="{ row }">
      <span>{{ formatAmount(row.total_price) }}</span>
    </template>

    <template #source_no="{ row }">
      <span>{{ row.source_no ?? '-' }}</span>
    </template>
