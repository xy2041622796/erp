...
import { useFormSchema } from '#/views/finance/payment/settlement/data';
import ContractPlanDetailDialog from '#/views/finance/payment/settlement/modules/contract-plan-detail-dialog.vue';
...
import SettlementItemForm from '#/views/finance/payment/settlement/modules/item-form.vue';
...
const formData = ref<ErpExpenseSettlementApi.ExpenseSettlement>();
const formType = ref('');
...
const purchaseOrderSelectRef = ref<InstanceType<typeof SaleOrderSelectDialog>>();
...
function buildUnifiedDisplayItem(params: {
  ...
  return {
    ...
    specification:
      String(params.specification || productMeta?.model || params.remark || '-').trim() || '-',
...
function mapContractRowsToDisplayItems(rows: any[], sourceNo: string) {
  return (rows || []).map((row: any, index: number) =>
    buildUnifiedDisplayItem({
      rowid: row?.rowid || row?.id,
      productId: row?.product_id || row?.purchase_order_id || row?.sale_order_id,
      fallbackName: row?.product_name || row?.purchase_order_no || row?.sale_order_no || `产品${index + 1}`,
...
function mapPurchaseOrderToDisplayItems(order: any) {
  ...
  return items.map((item: any, index: number) => {
    ...
    const amount =
      ...
      Math.max(moneyNumber(subMoney(item?.total_price, item?.tax_price)), 0) ||
      Math.max(moneyNumber(subMoney(item?.total_tax_price, item?.tax_price)), 0);
    const taxAmount =
    ...
    const totalPrice =
      toNumber(item?.total_price, 0) || toNumber(item?.total_tax_price, 0) || moneyNumber(addMoney([amount, taxAmount]));
    return buildUnifiedDisplayItem({
      ...
      num: item?.count,
      unitPrice: item?.product_price || (count > 0 ? moneyNumber(divMoney(amount, count)) : undefined),
      amount,
      taxAmount,
      totalPrice,
...
async function loadContractDisplayItems(contractId: string, contractName?: string) {
  ...
  await applyDisplayItems(mapContractRowsToDisplayItems(rows, contractName || selectedSubjectName.value || cid), { syncAmount: false });
}
...
async function loadOrderDisplayItems(orderId: string, orderNo?: string) {
  ...
  await ensureProductSimpleListLoaded();
  const detail = await getPurchaseOrder(oid);
  const sourceNo = String(orderNo || detail?.no || oid).trim();
  selectedOrderMeta.value = { id: oid, no: sourceNo };
  await applyDisplayItems(mapPurchaseOrderToDisplayItems(detail || { id: oid, no: sourceNo, items: [] }));
}
...
async function handleSupplierIdChange(v?: string) {
  ...
  await formApi.setValues({ supplier_id: v, project_id: undefined }, false);
...
const getTitle = computed(() => {
  const typeText = settlementType.value === 1 ? '退款结算' : '支出结算';
...
async function applyContractPlanAmount() {
  ...
  await formApi.setValues({ amount: planAmount, total_amount: planAmount }, false);
}
...
async function updateContractSummary(contractId: string) {
  ...
  try {
    ...
    const settlementsRes = await getExpenseSettlementPage({
    ...
    const settlements =
      (settlementsRes as any)?.list || (settlementsRes as any)?.items || (settlementsRes as any)?.Data || [];
    const validSettlementIds = new Set(
      (settlements || []).map((item: any) => String(item?.rowid || '').trim()).filter(Boolean),
    );
    const existingPlanIds = new Set(
      (rels || [])
        .filter((r: any) => validSettlementIds.has(String(r?.settlement_id || '').trim()))
        .map((r: any) => String(r?.plan_id || '').trim())
        .filter(Boolean),
    );
    const planCount = existingPlanIds.size;
    const currentId = formData.value?.rowid;
    const otherSettlements = settlements.filter((s: any) => s.rowid !== currentId);
    ...
    targetPlan.value = null;
    const sortedPlans = (plans || []).slice().sort((a: any, b: any) => Number(a.plan_period || 0) - Number(b.plan_period || 0));
    const currentPlan = currentPlanId.value
      ? sortedPlans.find((p: any) => String(p?.rowid || '').trim() === String(currentPlanId.value || '').trim())
      : null;
    if (currentPlan) {
      targetPlan.value = currentPlan;
    } else {
      targetPlan.value = sortedPlans.find((p: any) => !existingPlanIds.has(String(p?.rowid || '').trim()));
      if (!currentPlanId.value && targetPlan.value?.rowid) currentPlanId.value = String(targetPlan.value.rowid);
    }
...
function handleUpdateItems(items: ErpExpenseSettlementApi.ExpenseSettlementItem[]) {
  if (!formData.value) formData.value = { items: [] } as any;
  formData.value.items = items;
}
...
async function handleUpdateSummary(summary: { amount: number; taxAmount: number; total: number }) {
  ...
  formApi.setValues({ amount, total_amount: totalAmount });
}
...
async function buildPersistItems(values: any) {
  ...
  const orderId = String(values?.product_id || selectedOrderMeta.value?.id || '').trim();
  const orderNo = String(selectedOrderMeta.value?.no || selectedSubjectName.value || values?.product_name || '').trim();
  ...
  return [{ rowid: generateUUID(), product_id: orderId, product_name: orderNo, amount, total_tax_price: taxAmount, total_price: totalAmount, remark: values?.remark } as any];
}
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    modalApi.lock();
    try {
      const values = (await formApi.getValues()) as ErpExpenseSettlementApi.ExpenseSettlement;
      const effectiveItems = itemFormInstance?.getEffectiveItems?.() ?? formData.value?.items ?? values.items ?? [];
      const currentSubjectType = normalizeSubjectType((values as any).subject_type);
      const hasContract = hasText((values as any).contract_id);
      const hasOrder = hasText((values as any).product_id);
      ...
      const data: ErpExpenseSettlementApi.ExpenseSettlement = {
        ...(formData.value || {}),
        ...values,
        subject_type: currentSubjectType,
        contract_id: currentSubjectType === 'contract' ? (values as any).contract_id : undefined,
        product_id: currentSubjectType === 'order' ? (values as any).product_id : undefined,
        product_name: currentSubjectType === 'order' ? String(selectedOrderMeta.value?.no || selectedSubjectName.value || (values as any).product_name || '').trim() : undefined,
        items: persistItems,
        status: formType.value === 'create' ? 10 : (values as any).status,
      };
      if ((data as any).settlement_date) {
        const ts = Number((data as any).settlement_date);
        const date = Number.isNaN(ts) ? new Date((data as any).settlement_date) : new Date(ts);
        (data as any).settlement_date = formatDateTime(date);
      }
      if ((data as any).account_period) {
        const ts = Number((data as any).account_period);
        const date = Number.isNaN(ts) ? new Date((data as any).account_period) : new Date(ts);
        (data as any).account_period = formatDateTime(date);
      }
      const res = await (formType.value === 'create' ? createExpenseSettlement(data) : updateExpenseSettlement(data));
      if (
        formType.value === 'create' &&
        currentSubjectType === 'contract' &&
        hasText((data as any).contract_id) &&
        targetPlan.value &&
        (res as any)?.rowid
      ) {
        try {
          await createSettlementPlanRel(SETTLEMENT_MODEL_ID, {
            contract_id: (data as any).contract_id,
          ...
          console.error('Create expense settlement relation failed', error);
          ElMessage.warning('支出结算创建成功，但关联计划失败，请联系管理员');
        }
      }
      if (currentSubjectType === 'contract' && hasText((data as any).contract_id)) {
        try { await updateContract({ rowid: (data as any).contract_id, ConState: 2 } as any); } catch {}
      }
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    await ensureProductSimpleListLoaded();
    const data = modalApi.getData<{ contractId?: string; rowid?: string; settlement_type?: number; type: string }>();
    ...
    if (data?.contractId) {
      ...
      try {
        ...
        if (cProps) {
          selectedSubjectName.value = cProps.contract_name || cProps.contract_no || data.contractId;
          const supplier = (cProps as any).contract_party_b || (cProps as any).contract_party_a;
          if (supplier) {
            await formApi.setFieldValue('supplier_id', supplier);
            supplierId.value = String(supplier);
          }
        }
      } catch {}
    ...
    modalApi.lock();
    try {
      formData.value = await getExpenseSettlement(data.rowid!);
      if ((formData.value as any)?.settlement_type) settlementType.value = (formData.value as any).settlement_type;
      if (formData.value) {
        const r: any = formData.value;
        if ((!r.remark || String(r.remark).trim() === '') && r.description && String(r.description).trim() !== '') {
      ...
      await formApi.setValues({ ...(formData.value as any), subject_type: currentType }, false);
      salesmanId.value = (formData.value as any)?.salesman_id ? String((formData.value as any).salesman_id) : undefined;
      departId.value = (formData.value as any)?.depart_id ? String((formData.value as any).depart_id) : undefined;
      supplierId.value = (formData.value as any)?.supplier_id ? String((formData.value as any).supplier_id) : undefined;
      ...
      if (currentType === 'contract' && hasText((formData.value as any)?.contract_id)) {
        ...
        try {
          ...
          if (detail) contractName = detail.contract_name || detail.contract_no || contractId;
          currentPlanId.value = String((rel as any)?.plan_id || '');
        } catch (error) {
      ...
      } else if (formData.value && !(formData.value as any).items) {
        (formData.value as any).items = [];
      }
    } finally {
      modalApi.unlock();
    }
...
async function openSubjectSelect() {
  ...
  const currentSupplierId = await getCurrentSupplierId();
  if (!currentSupplierId) {
    ElMessage.warning('请先选择供应商');
    return;
  }
  if (subjectType.value === 'contract') {
    contractSelectModalApi.setData({ customerId: currentSupplierId });
    contractSelectModalApi.open();
    return;
  }
  purchaseOrderSelectRef.value?.open?.(currentSupplierId);
}
...
async function handleContractSelectConfirm(contract: any) {
  ...
  subjectType.value = 'contract';
  currentPlanId.value = '';
  await formApi.setValues({ subject_type: 'contract', contract_id: cid, product_id: undefined, product_name: undefined }, false);
  await updateContractSummary(cid);
  const pickedSupplierId = (contract as any).contract_party_b || (contract as any).contract_party_a;
  if (pickedSupplierId) {
    await formApi.setFieldValue('supplier_id', pickedSupplierId);
    supplierId.value = String(pickedSupplierId);
  }
...
async function handleOrderSelectConfirm(order: any) {
  ...
  await formApi.setValues({ subject_type: 'order', contract_id: undefined, product_id: orderId, product_name: orderNo }, false);
  if (order?.supplier_id || order?.supplier_id === 0) {
    await formApi.setFieldValue('supplier_id', order.supplier_id);
    supplierId.value = String(order.supplier_id);
  }
  await loadOrderDisplayItems(orderId, orderNo);
}
...
<template>
  ...
  >
    <Form class="mx-3">
      ...
      <template #depart_id>
        <ElInput :model-value="getDeptName(departId)" readonly placeholder="部门" class="!w-full" />
      </template>

      <template #subject_type_switch>
        <ElRadioGroup :model-value="subjectType" :disabled="formType === 'detail'" @update:model-value="handleSubjectTypeSwitch">
      ...
      <template #subject_selector>
        <div class="flex w-full cursor-pointer">
          <ElInput
            ...
            readonly
            :placeholder="subjectType === 'contract' ? '请选择支出合同' : '请选择采购订单'"
      ...
      <template #contract_detail_action>
        <div class="flex h-full items-center">
          <template v-if="subjectType === 'contract'">
            <ElButton type="primary" link @click="openContractPlanDetail">查看付款计划详情</ElButton>
            <span class="ml-2 text-xs text-gray-500">{{ contractSummaryText }}</span>
          </template>
          <template v-else>
            <span class="text-xs text-gray-500">已按采购订单加载产品信息</span>
          </template>
        </div>
      </template>
    ...
    <SaleOrderSelectDialog ref="purchaseOrderSelectRef" @confirm="handleOrderSelectConfirm" />
  </Modal>
</template>
