...
import type { OtherExpensePrintData } from '#/views/finance/print-templates/other-expense';
...
import { buildOtherExpensePrintHtml } from '#/views/finance/print-templates/other-expense';

import { useGridColumns, useGridFormSchema } from '#/views/finance/payment/other/data';
import Form from '#/views/finance/payment/other/modules/form.vue';
...
defineOptions({ name: 'ErpPaymentOtherExpense' });
...
function handleEdit(row: ErpOtherExpenseApi.OtherExpense) {
  formModalApi.setData({ type: 'edit', rowid: row.rowid }).open();
}

function handleDetail(row: ErpOtherExpenseApi.OtherExpense) {
...
async function handleUpdateStatus(row: any, status: number) {
  ...
  try {
    await updateOtherExpenseStatus(row.rowid, status);
...
const activeTab = ref<'all' | 'todoApprove' | 'todoPay' | 'done'>('all');
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues) => {
          return await getOtherExpensePage({
...
function mapPrintData(main: any): OtherExpensePrintData {
  return {
    ...
    payAccount: String(main.pay_account ?? ''),
    totalAmount: Number(main.total_amount ?? 0),
    payAmount: Number(main.pay_amount ?? 0),
    payBalance: Number(main.pay_balance ?? 0),
...
async function handlePrint(ids: string[]) {
  ...
  const list = await Promise.all(validIds.map((id) => getOtherExpense(id)));
  const printList = list.filter(Boolean).map((item) => mapPrintData(item));
  if (printList.length === 0) {
    ElMessage.warning('未找到可打印数据');
    return;
  }
  await printHtml(buildOtherExpensePrintHtml(printList));
}
</script>
  ...
  <Page auto-content-height>
    ...
    <div class="mb-2 flex items-center justify-between">
      <el-tabs v-model="activeTab" @tab-change="handleTabChange">
        ...
        <el-tab-pane label="待付款" name="todoPay" />
    ...
    <Grid table-title="其他支出列表">
      ...
      <template #supplier_project="{ row }">
        <div>
          <div class="text-primary">{{ getCustomerName(row.supplier_id) }}</div>
      ...
      <template #pay="{ row }">
        <div>
          ...
          <div>{{ row.pay_balance ?? '--' }}</div>
        </div>
      </template>

      <template #status="{ row }">
        <ElTag :type="row.status === 10 ? 'warning' : row.status === 20 ? 'info' : 'success'">
          {{ row.status === 10 ? '待审批' : row.status === 20 ? '待付款' : '已完成' }}
        </ElTag>
      </template>
        ...
        <TableAction
          :actions="[
            ...
            },
            {
              ...
              icon: ACTION_ICON.DELETE,
              popConfirm: {
                title: $t('ui.actionMessage.deleteConfirm', [row.settlement_no || '']),
