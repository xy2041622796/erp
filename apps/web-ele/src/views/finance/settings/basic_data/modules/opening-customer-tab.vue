...
defineOptions({ name: 'FinanceBasicDataOpeningCustomerTab' });
...
function recalcRow(row: CustomerOpeningRow) {
  ...
  row.opening_balance = moneyNumber(subMoney(receivable, received));
}
...
<template>
  <div class="opening-tab-page">
    <ElAlert
      title="当前已接入基础数据目录和客户主数据联动，正式审核入账仍需后端期初表承接。"
      type="warning"
      :closable="false"
    >
      <template #default>
        <div class="text-sm leading-6">
          本页先用于整理客户期初录入结构：客户、期初日期、应收、已收、余额。后续接入后端时，建议按“草稿
          / 审核”两阶段生效。
        </div>
      </template>
    </ElAlert>
    ...
    <ElTable v-else v-loading="loading" :data="rows" border>
      ...
      <ElTableColumn label="期初应收" min-width="140">
        <template #default="{ row }">
          <ElInputNumber
            v-model="row.opening_receivable"
            :precision="2"
            :controls="false"
            class="w-full"
