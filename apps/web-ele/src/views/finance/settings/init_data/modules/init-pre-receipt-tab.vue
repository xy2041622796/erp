...
import InitAdvanceForm from '#/views/finance/settings/init_data/modules/init-advance-form.vue';

import { ElMessage, ElTag } from 'element-plus';

defineOptions({ name: 'InitPreReceiptTab' });
...
let customerLoading: Promise<void> | null = null;
...
function getCustomerName(id: any) {
  ...
  const hit = customerOptions.value.find((c: any) => String(c?.rowid ?? c?.id ?? '') === key);
...
async function fillNames(rows: any[]) {
  await Promise.all([ensureCustomerLoaded(), ensureUserLoaded(), ensureDeptLoaded()]);
  await Promise.all(
    (rows || []).map(async (r: any) => {
      if (r?.supplier_id && !r.customer_name) r.customer_name = getCustomerName(r.supplier_id);
...
function handleCreate() {
  formModalApi.setData({ kind: 'pre_receipt', type: 'create' }).open();
}

function handleDetail(row: any) {
  formModalApi.setData({ kind: 'pre_receipt', type: 'detail', row }).open();
}

function handleEdit(row: any) {
  formModalApi.setData({ kind: 'pre_receipt', type: 'edit', row }).open();
}
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
    ...
    columns: [
      ...
      { title: '客户/项目', field: 'supplier_id', minWidth: 220, slots: { default: 'supplier_project' } },
      { title: '业务员/部门', field: 'salesman_id', minWidth: 180, slots: { default: 'sales_dept' } },
      { title: '预收金额', field: 'prepay_collect_amount', minWidth: 120 },
      { title: '期初预收余额', field: 'opening_balance', minWidth: 140 },
      ...
      <Grid table-title="期初预收款">
        ...
        <template #supplier_project="{ row }">
          <div class="flex flex-col">
            <div>{{ row.customer_name || row.supplier_id || '--' }}</div>
