...
import InitBusinessForm from '#/views/finance/settings/init_data/modules/init-business-form.vue';
...
function getSupplierName(id: any) {
  ...
  // 如果之前供应商列表未加载，可能缓存的是 id；当数据到位后需要纠正
  if (cached && cached !== key) return cached;

  // 没加载到供应商精简列表前，不要把 id 写入缓存
  if (supplierOptions.value.length === 0) return key;

  const hit = supplierOptions.value.find((c) => String(c?.rowid ?? c?.id ?? '') === key);
...
async function fillNamesForCurrentPage(rows: any[]) {
  ...
  await Promise.all(
    (rows || []).map(async (r) => {
      if (r?.customer_id && !r.customer_name) r.customer_name = getSupplierName(r.customer_id);
      if (r?.contract_id && !r.contract_name) r.contract_name = await getContractName(r.contract_id);
...
function handleCreate() {
  formModalApi.setData({ biz: 'expense', type: 'create' }).open();
}

function handleEdit(row: any) {
  formModalApi.setData({ biz: 'expense', type: 'edit', row }).open();
}
    ...
    <div class="min-h-0 flex-1">
      <Grid table-title="期初支出结算">
        ...
        <template #pay="{ row }">
          <div class="text-center">
            <div>
              {{ row.opening_received_paid_amount ?? row.received_amount ?? row.paid_received_amount ?? 0 }}
            </div>
            <div class="text-xs text-gray-500">
              {{ row.opening_balance ?? row.receivable_balance ?? row.after_balance ?? 0 }}
            </div>
          </div>
        </template>
