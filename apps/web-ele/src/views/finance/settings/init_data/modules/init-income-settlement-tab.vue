...
function getCustomerName(id: any) {
  ...
  // 如果之前客户列表未加载，可能缓存的是 id；当数据到位后需要纠正
  if (cached && cached !== key) return cached;

  // 没加载到客户精简列表前，不要把 id 写入缓存
  if (customerOptions.value.length === 0) return key;

  const hit = customerOptions.value.find((c) => String(c?.rowid ?? c?.id ?? '') === key);
...
async function fillNamesForCurrentPage(rows: any[]) {
  ...
  await Promise.all(
    (rows || []).map(async (r) => {
      if (r?.customer_id && !r.customer_name) r.customer_name = getCustomerName(r.customer_id);
...
function handleCreate() {
  formModalApi.setData({ biz: 'income', type: 'create' }).open();
}

function handleEdit(row: any) {
  formModalApi.setData({ biz: 'income', type: 'edit', row }).open();
}
...
<template>
  <div class="flex h-full flex-col">
    ...
    <div class="min-h-0 flex-1">
      <Grid table-title="期初收入结算">
        <template #date_no="{ row }">
          <div class="text-center">
            <div class="w-full">{{ formatDateOnly(row.opening_date ?? row.init_date ?? row.bill_date) || '--' }}</div>
        ...
        <template #receive="{ row }">
          <div class="text-center">
            <div>{{ row.opening_received_paid_amount ?? row.received_amount ?? row.paid_received_amount ?? 0 }}</div>
            <div class="text-xs text-gray-500">
