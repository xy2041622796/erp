...
function getSupplierName(id: any) {
  ...
  const hit = supplierOptions.value.find(
  ...
  await Promise.all(
    (rows || []).map(async (r) => {
      if (r?.customer_id && !r.customer_name)
        r.customer_name = getSupplierName(r.customer_id);
...
function handleCreate() {
  formModalApi.setData({ biz: 'other_expense', type: 'create' }).open();
}

function handleEdit(row: any) {
  formModalApi.setData({ biz: 'other_expense', type: 'edit', row }).open();
}
