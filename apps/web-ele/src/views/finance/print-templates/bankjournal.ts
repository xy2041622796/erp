...
export type BankjournalPrintRow = {
  date?: string;
  currencyName?: string;
  currency?: string;
  summary?: string;
...
export type BankjournalPrintData = {
  title?: string;
  companyName?: string;
  accountName?: string;
  period?: string;
...
type PrintColumn = {
  ...
  title: string;
  className?: string;
  getValue: (row: BankjournalPrintRow) => string;
};
...
function getPrintColumns(showAll?: boolean): PrintColumn[] {
  ...
  if (showAll) {
    columns.push(
      { key: 'counterpartyAccountNo', title: '往来单位账号', getValue: (row) => row.counterpartyAccountNo || '' },
      { key: 'counterpartyBankName', title: '往来单位开户行', getValue: (row) => row.counterpartyBankName || '' },
...
function hasPrintableColumnData(data: BankjournalPrintData, column: PrintColumn) {
...
function filterPrintableColumns(data: BankjournalPrintData, columns: PrintColumn[]) {
  return columns.filter((column) => hasPrintableColumnData(data, column));
}

function buildSummaryRow(columns: PrintColumn[], label: string, data: BankjournalPrintData) {
...
function buildOpeningRow(columns: PrintColumn[], data: BankjournalPrintData) {
...
function buildRows(data: BankjournalPrintData, columns: PrintColumn[]) {
...
export function buildBankjournalPrintHtml(data: BankjournalPrintData) {
      ...
      <table class="print-table journal-table">
        <thead>
          <tr class="journal-print-title-row">
            <th colspan="${columnCount}">${escapeHtml(data.title || '银行日记账')}</th>
          </tr>
  ...
  return buildPrintDocument(data.title || '银行日记账', body, `
