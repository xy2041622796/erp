...
import { useFormSchema } from '#/views/finance/bill/income/data';
import ItemForm from '#/views/finance/bill/income/modules/item-form.vue';
import SellerCompanySelect from '#/views/finance/bill/income/modules/seller-company-select.vue';
...
const getTitle = computed(() => {
  if (formType.value === 'create') {
    return $t('ui.actionTitle.create', ['开票信息']);
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    try {
      ...
      formData.value.details = formData.value.details.map((d: any) => ({
        ...d,
        product_num: d.product_num ?? 0,
      }));
      await formApi.setValues(formData.value as any, false);
      sellerCompanyId.value = (formData.value as any)?.seller_company_id;
    } finally {
