...
const [Modal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    try {
      ...
      // 加载初始用户数据
      await loadUserData(1, leftListState.value.pagination.page);
...
// 加载用户数据
async function loadUserData(pageNo: number, page: number) {
  try {
    ...
    leftListState.value.pagination.page = page;
...
function updateRightListData() {
  ...
  // 应用分页
  const { current, page } = rightListState.value.pagination;
...
async function handleLeftPaginationChange(page: number) {
  await loadUserData(page, leftListState.value.pagination.page);
}
...
async function handleDeptSelect(node: any) {
  ...
  // 重置分页并加载数据
  const { page } = leftListState.value.pagination;
  leftListState.value.pagination.current = 1;
  await loadUserData(1, page);
}
      ...
      <ElCol :span="18">
        ...
        <div class="mt-2 flex justify-between">
          <ElPagination
            ...
            v-model:page-size="leftListState.pagination.page"
          ...
          <ElPagination
            ...
            v-model:page-size="rightListState.pagination.page"
