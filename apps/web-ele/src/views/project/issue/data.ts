...
export function mapIssueTypeLabel(value: unknown) {
  const str = String(value ?? '').trim();
  return ISSUE_TYPE_OPTIONS.find((item) => item.value === str)?.label || str || '-';
}

export function mapIssuePriorityLabel(value: unknown) {
  const str = String(value ?? '').trim();
  return ISSUE_PRIORITY_OPTIONS.find((item) => item.value === str)?.label || str || '-';
}

export function mapIssueStatusLabel(value: unknown) {
  const str = String(value ?? '').trim();
  return ISSUE_STATUS_OPTIONS.find((item) => item.value === str)?.label || str || '-';
}
...
export const issueConfig: ProjectSubmoduleCrudConfig = {
  ...
  searchSchema: [
    ...
    { fieldName: 'title', label: '标题', component: 'Input', componentProps: { clearable: true, placeholder: '请输入标题' } },
    {
      fieldName: 'type', label: '类型', component: 'Select',
      componentProps: { options: ISSUE_TYPE_OPTIONS.map((item) => ({ ...item })), clearable: true },
    },
    {
  ...
  columns: [
    ...
    { title: '类型', field: 'type', minWidth: 100, align: 'center', slots: { default: 'cell_issue_type' } },
    { title: '优先级', field: 'priority', minWidth: 100, align: 'center', slots: { default: 'cell_issue_priority' } },
    { title: '状态', field: 'status', minWidth: 100, align: 'center', slots: { default: 'cell_issue_status' } },
    { title: '提出人ID', field: 'reporter_id', minWidth: 160 },
    { title: '处理人ID', field: 'assignee_id', minWidth: 160 },
    { title: '到期日期', field: 'due_date', minWidth: 120 },
