...
function getPopConfirmProps(attrs: PopConfirm) {
  const { title, okText, cancelText, confirm, cancel, icon, disabled } =
    attrs;
  return {
    title,
    ...
    icon,
    disabled,
    onConfirm: isFunction(confirm) ? confirm : undefined,
    onCancel: isFunction(cancel) ? cancel : undefined,
  };
}
