...
const hideHeaderNav = computed(() => {
  return !!route.meta?.hideHeaderNav;
});

const hideSidebarNav = computed(() => {
  return !!route.meta?.hideSidebarNav;
});
...
const resolvedLogoText = computed(() => props.logoText || preferences.app.name);
const resolvedLogoSrc = computed(() => props.logoSrc || preferences.logo.source);
const resolvedLogoSrcDark = computed(() => props.logoSrcDark || preferences.logo.sourceDark || resolvedLogoSrc.value);

const resolvedMenus = computed(() => {
  if (hideSidebarNav.value) {
    return [];
  }
...
<template>
  <VbenAdminLayout
    ...
    :sidebar-enable="!hideSidebarNav && sidebarVisible"
