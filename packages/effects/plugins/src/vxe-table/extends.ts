...
function extendProxyOption(
  ...
  const wrapperFn = async (
    ...
    // 兼容多种返回格式：支持直接返回数组、{ list, total }、以及标准的 { items, total }
    if (Array.isArray(data)) {
      return { items: data, total: data.length } as any;
    }
    if (
      data &&
      (data as Recordable<any>).list &&
      !(data as Recordable<any>).items
    ) {
      (data as Recordable<any>).items = (data as Recordable<any>).list;
    }
    return data;
  };
  api.setState({
