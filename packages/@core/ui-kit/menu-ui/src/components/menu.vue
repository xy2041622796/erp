...
function findFirstChildMenuItem(path: string) {
  return Object.values(items.value).find((item) =>
    item.parentPaths?.includes(path),
  );
}

function selectFirstChildMenuItem(path: string) {
  const child = findFirstChildMenuItem(path);
  if (!child?.path) {
    return;
  }

  activePath.value = child.path;
  emit('select', child.path, child.parentPaths || []);
}
...
function handleSubMenuClick({ parentPaths, path }: MenuItemRegistered) {
  ...
  if (props.autoActivateChild) {
    selectFirstChildMenuItem(path);
  }
}
