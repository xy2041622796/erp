...
- 页面能力：现金日记账查询、逐行新增/保存/删除、点击行进入编辑、打印列表、打印凭据、按当前账套与当前现金账户导入导出、关联凭证、取消关联、生成凭证。
- 顶部布局：`journal-toolbar` 紧凑工具栏；左侧为现金账户和日期范围筛选，右侧为新增、显示全部、打印、导入导出、批量、关联凭证、生成凭证等操作；宽屏单行展示，空间不足时自然换行。
...
- 后端保护：`saveCashdayRow` 和 `deleteCashdayRow` 调用 `assertPeriodNotCarriedForwardByDate` 兜底校验；即使绕过前端按钮，只要该日记账日期所在月份已结转，也禁止新增、修改或删除现金日记账明细。
- 导出能力：使用 ExcelJS 导出与模板一致的“现金日记账”主表字段：日期、摘要、收支类别、往来单位、收入借方、支出贷方、余额、关联凭证、日记账序号；导入说明页写入当前账套和当前现金账户；隐藏“系统信息”页写入 `account_set_id`、`account_set_name`、`cash_account_id`、`cash_account_name`、`period`。
- 导入能力：读取首个工作表，按模板字段导入；导入前必须已选择账套与现金账户；如果文件带有“系统信息”页且 `account_set_id` 与当前账套不一致，则阻止导入并提示切换账套；保存时统一写入当前现金账户，由 `saveCashdayRow` 根据资金账户回填 `account_set_id` 和企业字段，确保数据进入当前账套；导入保存同样受已结转月份限制。
- 编辑行为：通过 `editingRowKey`、`isEditing(row)`、`onRowClick(row)` 控制当前行编辑态；新增行默认进入编辑，保存/删除/重载后退出编辑。
- 使用到的数据接口：`fetchCashAccounts`、`fetchCashdayList`、`fetchIoTypes`、`fetchCounterparties`、`saveCashdayRow`、`deleteCashdayRow`、`linkCashdayVoucher`、`unlinkCashdayVoucher`、`getVoucherPage`、`getVoucherDetails`、`getCarriedForwardPeriodStatusByDate`、`assertPeriodNotCarriedForwardByDate`。
- 使用到的状态：`useAccountSetStore` 用于读取当前账套 ID 与名称，导入导出均以当前账套作为边界。
- 适用场景：统一现金日记账顶部工具栏布局和主题色，保持与银行日记账页面一致，并避免跨账套导入导出及已结转月份继续变更明细。
...
- 期初余额仅第一页展示与满页自动顺延：
  - 修改文件：`apps/web-ele/src/views/finance/funds/cashday/index.vue`。
  - 规则：`初始化余额/期初余额` 行只在现金日记账第 1 页展示，第 2 页及后续页不再展示该行。
  - 分页口径：期初行占用第 1 页的一个展示行；第 1 页最多展示 `pageSize - 1` 条明细，明细满页后剩余数据自动顺延到第 2 页及后续页，无需手动额外处理数据截取。
  ...
  - 影响范围：仅影响现金日记账前端分页展示与分页总数；接口、保存、删除、导入导出、凭证关联和生成凭证逻辑不变。
...
- 表格编辑目标行自动切页：
  ...
  - 影响范围：仅影响现金日记账表格自动切页与编辑聚焦；分页组件手动切页、保存、删除、导入导出、凭证关联和生成凭证逻辑不变。
